...
kal_bool temp_error_recovery_chr_flag = KAL_TRUE;
 ...
 kal_bool    g_temperature_limit = KAL_FALSE;
 kal_uint32 g_terminal_voltage = CHARGING_FULL_VOLTAGE;
 kal_int32 g_terminal_current = CHARGING_FULL_CURRENT;
 kal_int32 g_terminal_current_base = 0;
 
    #if defined (HIGH_BATTERY_VOLTAGE_SUPPORT)
    kal_uint32 g_charging_voltage_value = BATTERY_VOLT_04_340000_V;
    static HV_CHR_STATE_ENUM g_temperature_state = CHARGER_TEMPERATURE_NORMAL;
    #else
    kal_uint32 g_charging_voltage_value = BATTERY_VOLT_04_200000_V;
...
void select_charging_curret(void)
{
    ...
    #if defined (HIGH_BATTERY_VOLTAGE_SUPPORT) && defined (TINNO_BATTERY_FEATURE)
    switch(g_temperature_state)
    {
        case CHARGER_TEMPERATURE_CASE1_LOWER:
        {
            // CASE1: ICharging=0.15C, ITerm=0.02C
            if ( BMT_status.charger_type != STANDARD_HOST )
            {
                if (HV_CHARGER_CASE1_CURRENT < CHARGE_CURRENT_500_00_MA)
                {
                    g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA; 
                    g_temp_CC_value = HV_CHARGER_CASE1_CURRENT;
                }
                else
                {
                    g_temp_input_CC_value = HV_CHARGER_CASE1_CURRENT; 
                    g_temp_CC_value = HV_CHARGER_CASE1_CURRENT;
                }
            }
            g_terminal_current = HV_CHARGER_CASE1_TERMINAL_CURRENT;
        }
        break;
        
        case CHARGER_TEMPERATURE_NORMAL:
        {
            // ICharging=0.5C, ITerm=0.02C
            g_terminal_current = CHARGING_FULL_CURRENT;
        }
        break;
        
        case CHARGER_TEMPERATURE_CASE2_HIGHER:
        {
            // CASE2: ICharging=0.3C, ITerm=0.02C
            if ( BMT_status.charger_type != STANDARD_HOST )
            {
                g_temp_input_CC_value = HV_CHARGER_CASE2_CURRENT; 
                g_temp_CC_value = HV_CHARGER_CASE2_CURRENT;
            }
            g_terminal_current = HV_CHARGER_CASE2_TERMINAL_CURRENT;
        }
        break;
        
        case CHARGER_TEMPERATURE_LOWEST:
        case CHARGER_TEMPERATURE_HIGHEST:
        default:
        {
            // Disable charging.
            g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA; 
    ...
    battery_log(BAT_LOG_CRTI, "[BATTERY] After HV check. CC input: %dmA CC: %dmA term: %dmA @%d.C\r\n", 
        g_temp_input_CC_value/100, g_temp_CC_value/100, g_terminal_current, BMT_status.temperature);
    #endif  /* HIGH_BATTERY_VOLTAGE_SUPPORT && TINNO_BATTERY_FEATURE */

#if defined (TINNO_USB_IF)
    g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
    g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
#endif  /* TINNO_USB_IF */	
}
...
void set_hv_charging_state(void)
{
    if (HV_CHARGER_CASE1_TEMPERATURE_MIN > BMT_status.temperature)
    {
        g_temperature_state = CHARGER_TEMPERATURE_LOWEST;
    }
    else if ((HV_CHARGER_CASE1_TEMPERATURE_MIN <= BMT_status.temperature) 
        && (HV_CHARGER_CASE1_TEMPERATURE_MAX >= BMT_status.temperature))
    {
        // ICharging=0.15C, ITerm=0.02C
        g_temperature_state = CHARGER_TEMPERATURE_CASE1_LOWER;
    }   
    else if ((HV_CHARGER_CASE1_TEMPERATURE_MAX < BMT_status.temperature) 
        && (HV_CHARGER_CASE2_TEMPERATURE_MIN > BMT_status.temperature))
    {
        // ICharging=0.5C, ITerm=0.02C
        if ((CHARGER_TEMPERATURE_CASE2_HIGHER == g_temperature_state)
            && (HV_CHARGER_CASE2_TEMPERATURE_MIN-3 < BMT_status.temperature))
        {
            // Dejitter, don't change state.
        }
        else
        {
            g_temperature_state = CHARGER_TEMPERATURE_NORMAL;
        }
    }
    else if ((HV_CHARGER_CASE2_TEMPERATURE_MIN <= BMT_status.temperature) 
        && (HV_CHARGER_CASE2_TEMPERATURE_MAX >= BMT_status.temperature))
    {
        // ICharging=0.3C, ITerm=0.02C
        if ((CHARGER_TEMPERATURE_HIGHEST == g_temperature_state)
            && (HV_CHARGER_CASE2_TEMPERATURE_MAX-3 < BMT_status.temperature))
        {
            // Dejitter, don't change state.
        }
        else
        {
            g_temperature_state = CHARGER_TEMPERATURE_CASE2_HIGHER;
        }
    }
    else    // Lowwer than 0 degree or higher than 60 degree
    {
        // Disable charging.
        g_temperature_state = CHARGER_TEMPERATURE_HIGHEST;
    }
    battery_log(BAT_LOG_CRTI, "[BATTERY] After HV check. g_temperature_state %d @%d.C\n",
        g_temperature_state, BMT_status.temperature);
}

void select_charging_ov_hv(void)
{
    g_temperature_limit = KAL_FALSE;
    ...
    switch(g_temperature_state)
    {
        ...
        {
            // ICharging=0.15C, ITerm=0.02C
            g_charging_voltage_value = HV_CHARGER_CASE1_VOLTAGE;
            g_terminal_voltage = HV_CHARGER_CASE1_TERMINAL_VOLTAGE;
        }
        break;
        ...
        {
            // ICharging=0.5C, ITerm=0.02C
            g_charging_voltage_value = HV_CHARGER_VOLTAGE;
            g_terminal_voltage = CHARGING_FULL_VOLTAGE;
        }
        break;
        ...
        {
            // ICharging=0.3C, ITerm=0.02C
            g_charging_voltage_value = HV_CHARGER_CASE2_VOLTAGE; 
            g_terminal_voltage = HV_CHARGER_CASE2_TERMINAL_VOLTAGE;
            g_temperature_limit = KAL_TRUE;
        }
        break;
        ...
        {
            // Disable charging.
            g_charging_voltage_value = BATTERY_VOLT_03_600000_V; 
            g_terminal_voltage = 3600;
        }
        break;
    }

    battery_log(BAT_LOG_CRTI, "[BATTERY] After HV check. charging voltage %dmV, turn off voltage %dmV @%d.C\n",
        g_charging_voltage_value/1000, g_terminal_voltage, BMT_status.temperature);
}
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
        ...
        #if defined (TINNO_BATTERY_FEATURE) && defined (HIGH_BATTERY_VOLTAGE_SUPPORT)
        set_hv_charging_state();
        #endif /* TINNO_BATTERY_FEATURE && HIGH_BATTERY_VOLTAGE_SUPPORT*/
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
			...
			/*Set CV Voltage */
                ...
                #if defined (TINNO_BATTERY_FEATURE)
                    #if defined (HIGH_BATTERY_VOLTAGE_SUPPORT)
                    select_charging_ov_hv();
                    #endif  /* HIGH_BATTERY_VOLTAGE_SUPPORT */
                    cv_voltage = g_charging_voltage_value;
                #else
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
			cv_voltage = BATTERY_VOLT_04_340000_V;
#else
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
    ...
    #if defined (TINNO_BATTERY_FEATURE)
    battery_log(BAT_LOG_CRTI, "[BATTERY] CC bat_vol=%dmV/%dmV, ICharging=%dmA/(%dmA+%dmA), Temp=%d.C, limit=%d\n", 
        BMT_status.bat_vol, g_terminal_voltage, 
        BMT_status.ICharging, g_terminal_current, g_terminal_current_base,
        BMT_status.temperature, g_temperature_limit);

    #if defined (TINNO_USB_IF)
    return PMU_STATUS_OK;
    #endif  /* TINNO_USB_IF */

    if ((BMT_status.bat_vol >= g_terminal_voltage)
        && (BMT_status.ICharging < (g_terminal_current + g_terminal_current_base))
        && !g_temperature_limit)
    #else
...
#if defined (TINNO_BATTERY_FEATURE)
    if (RECHARGING_VOLTAGE <= BMT_status.bat_vol)
    {
        kal_uint32 charging_enable = KAL_FALSE; /*  Disable charger */
        battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
    }
    else
#else
