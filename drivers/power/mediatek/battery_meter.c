...
u32 g_battery_init_voltage = 0;
u32 g_battery_init_current = 0;
u32 g_battery_init_charger = 0;
...
/* ============================================================ // */
#if defined (TINNO_BATTERY_FEATURE) && defined (CONFIG_OF)
#define ATAG_BATTERY       0x41000809
struct tag_battery 
{    	
    u32 size;
    u32 tag;
...
static int __init dt_get_lk_bat_param(unsigned long node, const char *uname, int depth, void *data)
{
    struct tag_battery *tags = NULL;

    if (depth != 1 || (strcmp(uname, "chosen") != 0 && strcmp(uname, "chosen@0") != 0))
        return 0;

    tags = of_get_flat_dt_prop(node, "atag,battery", NULL);
    if (tags)
    {
        g_battery_init_voltage = tags->bat_voltage;            
        g_battery_init_current = tags->bat_current;            
        g_battery_init_charger = tags->bat_charger;            
        bm_print(BM_LOG_CRTI, "LK battery vol=%dmV current=%dmA charger=%d\n",                 
            g_battery_init_voltage, g_battery_init_current, g_battery_init_charger);  
    }
    else
    {
        bm_print(BM_LOG_CRTI, "LK atag,battery fail\n"); 
    }

    /* break now */
    return 1;
}
#endif  /* TINNO_BATTERY_FEATURE && CONFIG_OF */
...
int BattThermistorConverTemp(int Res)
{
	...
	int TBatt_Value = -200, TMP1 = 0, TMP2 = 0;
    #if defined (TINNO_BATTERY_FEATURE)
    int TTableSize = sizeof(Batt_Temperature_Table) / sizeof(BATT_TEMPERATURE);
    #endif  /* TINNO_BATTERY_FEATURE */

    #if defined (TINNO_BATTERY_FEATURE)
    if(Res>=Batt_Temperature_Table[0].TemperatureR)
    {
        TBatt_Value = -20;//Batt_Temperature_Table[0].BatteryTemp; //-30;
    }
    else if(Res<=Batt_Temperature_Table[TTableSize-1].TemperatureR)
    {
        TBatt_Value = 60; //Batt_Temperature_Table[TTableSize-1].BatteryTemp; // 80;
    }
    else
    #endif  /* TINNO_BATTERY_FEATURE */
    {
    ...
    bm_print(BM_LOG_CRTI, "BattThermistorConverTemp ,%d,Ohm, %d,C\n", Res, TBatt_Value);
...
{    
    ...
    for (i = 0; i < saddles - 1; i++)
    {
        ...
        {
            #if defined (TINNO_BATTERY_FEATURE)
            if ( ((profile_p+i+1)->percentage) == ((profile_p+i)->percentage) )
            {
...
void dod_init(void)
{
    ...
    bm_print(BM_LOG_CRTI, "[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d, g_boot_reason=%d, g_boot_mode=%d\n", 
        gFG_voltage, gFG_capacity_by_v, gFG_capacity_by_v_init, g_boot_reason, g_boot_mode);
    tinno_log_printk(BM_LOG_CRTI, "[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC=%d, g_boot_reason=%d, g_boot_mode=%d\n", 
        gFG_voltage, gFG_capacity_by_v, gFG_capacity_by_v_init, g_boot_reason, g_boot_mode);
...
#endif
	{
        ...
        if (!g_battery_init_charger 
            && ((g_rtc_fg_soc-gFG_capacity_by_v) >= 5)
            && !(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT 
                || g_boot_reason == BR_TOOL_BY_PASS_PWK || g_boot_reason == BR_2SEC_REBOOT 
                || g_boot_mode == RECOVERY_BOOT)) // discharging
        {
            gFG_capacity_by_v = g_rtc_fg_soc - 1; 
        }
        //else if(g_battery_init_charger 
        //    && ((gFG_capacity_by_v-g_rtc_fg_soc) >= 5)) // charging
        //{
        //    gFG_capacity_by_v = g_rtc_fg_soc + 1;
        //}
        else
	...
	bm_print(BM_LOG_CRTI, "[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		 g_rtc_fg_soc, gFG_capacity_by_v);
    tinno_log_printk(BM_LOG_CRTI, "[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n", 
            g_rtc_fg_soc, gFG_capacity_by_v);
...
/* 1.1 Average FG_voltage */
    ...
    #if defined (TINNO_BATTERY_FEATURE)
    bm_print(BM_LOG_CRTI, "LK charger:%d, voltage:%dmV, (%d, %d)\n", 
        g_battery_init_charger, g_battery_init_voltage, 
        gFG_voltage, gFG_vbat_offset);
    #endif  /* TINNO_BATTERY_FEATURE */
...
/* 1. Get Raw Data */
    ...
    #if defined (TINNO_BATTERY_FEATURE)
    // Used LK voltage to get init capacity.
    bm_print(BM_LOG_CRTI, "Jake [FGADC] LK charger:%d, voltage:%dmV, current:%dmA\n", 
        g_battery_init_charger, g_battery_init_voltage, g_battery_init_current);
    tinno_log_printk(BM_LOG_CRTI, "[FGADC] LK charger:%d, voltage:%dmV, current:%dmA\n", 
        g_battery_init_charger, g_battery_init_voltage, g_battery_init_current);
    if (g_battery_init_charger)
    {
        if (2500 < g_battery_init_voltage)  // If g_battery_init_voltage less 2500mV, maybe error.
        {
            gFG_voltage = g_battery_init_voltage;
            gFG_current = g_battery_init_current*10;
            gFG_Is_Charging = KAL_FALSE;
        }
    }
    #endif  /* TINNO_BATTERY_FEATURE */
    ...
    tinno_log_printk(BM_LOG_CRTI, "[FGADC] SWOCV : %d,%d,%d,%d,%d,%d\n", 
...
kal_int32 battery_meter_get_charging_current(void)
{
	...
	return 0;
#elif defined (TINNO_BATTERY_FEATURE)
//#elif !defined (SWCHR_POWER_PATH)	// to be check
...
#if defined (TINNO_BATTERY_FEATURE)
    g_terminal_current_base = CHARGING_TERMINAL_CURRENT_BASE;
#endif  /* TINNO_BATTERY_FEATURE */
    ...
    bm_print(BM_LOG_CRTI, "[g_Get_I_Charging] gFG_current_AVG=%dmA, gFG_current=%dmA, ICharging=%dmA\r\n", gFG_current_AVG/10, gFG_current/10, ICharging);

    #if defined (SWCHR_POWER_PATH)
    return gFG_current_AVG/10;
    #else
    return ICharging;	
    #endif  /* SWCHR_POWER_PATH */
#else
    return 0;
#endif	
}
...
kal_int32 battery_meter_initial(void)
{
	...
	if (meter_initilized == KAL_FALSE) {
        #if defined (TINNO_BATTERY_FEATURE) && defined (CONFIG_OF)
        of_scan_flat_dt(dt_get_lk_bat_param, NULL);
        #endif  /* TINNO_BATTERY_FEATURE && CONFIG_OF */
...
kal_int32 battery_meter_get_tempR(kal_int32 dwVolt)
{
    ...
    #if defined (TINNO_BATTERY_FEATURE)
    if (RBAT_PULL_UP_VOLT == dwVolt)
    {
        #if (BAT_NTC_10 == 1)
        TRes = 10000;  // NTC resistor isn't exist. Simultate to 25.C
        #endif  /* (BAT_NTC_10 == 1) */
        
        #if (BAT_NTC_47 == 1)
        TRes = 47000;  // NTC resistor isn't exist. Simultate to 25.C
        #endif  /* (BAT_NTC_47 == 1) */
    }
    else
    #endif  /* TINNO_BATTERY_FEATURE */
...
#if defined (TINNO_SMART_SYNC_CAP)
static kal_int32 tracking_point_tbl[] = {70, 12, 1};
kal_bool battery_meter_nPercent_param_sync(void)
{
    static kal_int32 last_tracking_point = CUST_TRACKING_POINT;
    static kal_int32 last_temp = 25;
    kal_bool needUpdate = KAL_FALSE;
    kal_int32 i, tbl_size = sizeof(tracking_point_tbl)/sizeof(kal_int32);
    
    bm_print(BAT_LOG_CRTI, "battery_meter_nPercent_param_sync, last_tracking_point(%d), UI_SOC(%d) \n",
        last_tracking_point, BMT_status.UI_SOC);

    // Find tracking point.
    for (i=0; i<tbl_size; i++)
    {
        if (BMT_status.UI_SOC >= tracking_point_tbl[i])
        {
            if (last_tracking_point != tracking_point_tbl[i])
            {
                needUpdate = KAL_TRUE;
                last_tracking_point = tracking_point_tbl[i];
            }
            break;
        }
    }

    // Check temperture change.
    if (last_temp != BMT_status.temperature)
    {
        last_temp = BMT_status.temperature;
        needUpdate = KAL_TRUE;
    }
    
    // Update relate parameter.
    if (needUpdate) // update for temperature change.
    {
        g_tracking_point = last_tracking_point;
        gFG_15_vlot = fgauge_read_v_by_capacity(100 - last_tracking_point);
        BMT_status.nPercent_ZCV = battery_meter_get_battery_nPercent_zcv();
    	BMT_status.nPrecent_UI_SOC_check_point = battery_meter_get_battery_nPercent_UI_SOC();

        bm_print(BAT_LOG_CRTI, "battery_meter_nPercent_param_sync end, last_tracking_point(%d), g_tracking_point(%d), gFG_15_vlot(%d), nPercent_ZCV(%d)\n",
            last_tracking_point, g_tracking_point, gFG_15_vlot, BMT_status.nPercent_ZCV);
    }
