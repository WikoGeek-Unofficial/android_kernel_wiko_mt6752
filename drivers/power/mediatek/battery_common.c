...
static int low_power_count = 0; // TINNO_A_S0050
...
#if defined (TINNO_POWER_SUPPLY_PROP_DEBUG)
static const char POWER_SUPPLY_PROP_tbl_s[][64] = 
{
...
#endif  /* TINNO_POWER_SUPPLY_PROP_DEBUG */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
    ...
    #if defined (TINNO_POWER_SUPPLY_PROP_DEBUG)
    battery_log(BAT_LOG_CRTI, "battery_get_property %d, %s, %d\n", ret, &POWER_SUPPLY_PROP_tbl_s[psp][0], val->intval);
    #endif  /* TINNO_POWER_SUPPLY_PROP_DEBUG */
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	#if defined (TINNO_SMART_SYNC_CAP)
	static kal_uint32 timer_counter = 1;	
    #else
	static kal_uint32 timer_counter = (NPERCENT_TRACKING_TIME/BAT_TASK_PERIOD);	
	#endif  /* TINNO_SMART_SYNC_CAP */
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if(chr_wake_up_bat==KAL_FALSE)
		{
			...
			{
                #if defined (TINNO_BATTERY_FEATURE)
                if (KAL_FALSE == BMT_status.charger_exist)  // UI_SOC drop only no charger.
                #endif  /* TINNO_BATTERY_FEATURE */
                {
                    BMT_status.UI_SOC--;
                }
		...
		#if !defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
        #if defined (TINNO_BATTERY_FEATURE)
        if ((KAL_FALSE == BMT_status.charger_exist) && (BMT_status.UI_SOC < BMT_status.SOC)) // UI_SOC increase only charger exist.
        {
            battery_log(BAT_LOG_CRTI, "keep UI_SOC to %d, %d\n", BMT_status.UI_SOC, BMT_status.SOC);
        }
        else
        #endif  /* TINNO_BATTERY_FEATURE */
        {
        ...
        #if defined (TINNO_BATTERY_FEATURE)
        if ((100 == BMT_status.UI_SOC) && (KAL_FALSE == BMT_status.bat_full) && BMT_status.charger_exist)
        {
            BMT_status.UI_SOC = 99; // charging is not full,  UI keep 99%
            battery_log(BAT_LOG_CRTI, "[Sync_Real] keep UI_SOC to 99\n");
        }
        #endif  /* TINNO_BATTERY_FEATURE */
		#else
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if( (BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR) ) {
        #if defined (TINNO_BATTERY_FEATURE)
        // Jake.L, DATE20140307, TINNO_A_S0050, DATE20140307-01 LINE
        low_power_count = 0;
        #endif  /* TINNO_BATTERY_FEATURE */
        ...
        {
            resetBatteryMeter = mt_battery_0Percent_tracking_check();
            // Jake.L, DATE20140307, TINNO_A_S0050, DATE20140307-01 START
            #if defined (TINNO_BATTERY_FEATURE) && !defined (TINNO_BATTERY_LOW_DEBUG)
            if (BMT_status.UI_SOC <= 0)
            {
                if (BMT_status.bat_vol <= (SYSTEM_OFF_VOLTAGE-100))
                {
                    low_power_count++;
                    printk("There is no charger? %d\n", low_power_count);
        ...
        {      
            ...
            #endif  /* TINNO_BATTERY_FEATURE */
            #if defined (TINNO_SMART_SYNC_CAP)
            // Update nPercent step
            battery_meter_nPercent_param_sync();
            #endif  /* TINNO_SMART_SYNC_CAP */
    ...
    #if defined (TINNO_BATTERY_LOW_DEBUG)
    if (BMT_status.UI_SOC < 1)
    {
        BMT_status.UI_SOC = 1;
        battery_log(BAT_LOG_CRTI, "Keep 1%% for battery test.\n");
    }
    #endif  /* TINNO_BATTERY_LOW_DEBUG */
...
void mt_battery_GetBatteryData(void)
{
	...
	#if defined (TINNO_BATTERY_FEATURE) && defined(TINNO_BATTERY_NOTIFY_VCHARGER)
	if( upmu_is_chr_det() == KAL_TRUE ) {
	...
	charger_vol = battery_meter_get_charger_voltage();
    if (V_CHARGER_MIN_THL > charger_vol)
        charger_vol = 0;
    #else
	if( upmu_is_chr_det() == KAL_TRUE ) {
		ICharging = battery_meter_get_charging_current();
...
static void mt_battery_CheckBatteryStatus(void)
{
    ...
    // Jake.L, DATE20130710, Resume charging while no error, DATE20130710-01 LINE
    #if defined (TINNO_BATTERY_FEATURE)
    battery_log(BAT_LOG_CRTI, "[BATTERY] Jake.L BMT_status.bat_charging_state=0x%x\n\r", BMT_status.bat_charging_state); 
...
static void mt_battery_notify_VBatTemp_check(void)
{
    ...
    if (BMT_status.charger_exist != KAL_TRUE)
    {
        battery_log(BAT_LOG_CRTI, "[BATTERY] Don't check temperature while no charger.\n");		
        return ;
    }
    #endif  /* TINNO_BATTERY_FEATURE */
...
static void mt_battery_notify_VCharger_check(void)
{
    ...
    #if defined (TINNO_BATTERY_FEATURE) && defined (TINNO_BATTERY_NOTIFY_VCHARGER)
    if((V_CHARGER_MIN_THL < BMT_status.charger_vol) && (BMT_status.charger_vol < V_CHARGER_MIN))
    {
        g_BatteryNotifyCode |= 0x0040;
        battery_log(BAT_LOG_CRTI, "[BATTERY] BMT_status.charger_vol(%ld) < %d mV\n", BMT_status.charger_vol, V_CHARGER_MIN);
    }
    else
    {
        g_BatteryNotifyCode &= ~(0x0040);
...
static void mt_battery_thermal_check(void)
{
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
		if (BMT_status.temperature >= 75/*(60+10)*/) {
#if defined(CONFIG_POWER_EXT)
			battery_log(BAT_LOG_CRTI,
...
void BAT_thread(void)
{
    ...
    battery_log(BAT_LOG_CRTI, "Battery Info (UI_SOC, SOC, Vol, T, C),%d,%d,%d,%d,%d\n",
        BMT_status.UI_SOC, BMT_status.SOC, BMT_status.bat_vol, BMT_status.temperature, BMT_status.charger_exist);
    
    // Jake.L, DATE20130820, SSEO-118, DATE20130820-01 START    
    #if defined (TINNO_BATTERY_FEATURE)
    if (FACTORY_BOOT == g_boot_mode)    
    {        
        if( BMT_status.charger_exist == KAL_TRUE )        
        {            
            if ((BMT_status.UI_SOC >= 100/*90*/))             
            {                
...
int charger_hv_detect_sw_thread_handler(void *unused)
    ...
    #if defined (TINNO_BATTERY_FEATURE)
    // VTHH fix to 6.5V
    kal_uint32 hv_voltage = /*V_CHARGER_MAX*/6500*1000;
    #else
...
#if defined (TINNO_POWEROFF_CHARGER_DEBUG)
#define TINNO_DEBUG_BUFFER_LEN  8*1024
static char g_bat_dbg_buff[TINNO_DEBUG_BUFFER_LEN]; // = "tinno battery log\n";
static char *g_bat_dbg_buff_wptr=g_bat_dbg_buff;
static int g_file_pos = 0;

int Enable_TINNO_LOG = 5;

int __tinno_printk(const char *fmt_str, ...)
{
    va_list args;
    int r;

    if (g_bat_dbg_buff_wptr >= (g_bat_dbg_buff + TINNO_DEBUG_BUFFER_LEN - 128))
    {
        r = sprintf(g_bat_dbg_buff_wptr, "TINNO Log buffer over!!!\n");
        return 0;
    }
    
    va_start(args, fmt_str);
    //r = sprintf(g_bat_dbg_buff_wptr, fmt_str, args);
    r = vsnprintf(g_bat_dbg_buff_wptr, (g_bat_dbg_buff + TINNO_DEBUG_BUFFER_LEN - g_bat_dbg_buff_wptr), fmt_str, args);
    va_end(args);

    g_bat_dbg_buff_wptr += r;
    return r;
}

static ssize_t battery_dbgbuff_read(struct file *file, char *buffer, size_t count, loff_t *data)
{
    kal_uint32 buffLen = 0;

    if (!buffer) return 0;
    
    battery_log(BAT_LOG_CRTI, "battery_dbgbuff_read g_file_pos=%d, count=%d\n", g_file_pos, count);

    if (0 == g_file_pos)    // get size.
    {
        buffLen = strlen((const char *)g_bat_dbg_buff);
        copy_to_user((void *)buffer, (const void *)&buffLen, 4);
        g_file_pos += 4;

        return 4;
    }
    else    // get buffer content.
    {
        char *bat_dbg_buff_rptr = g_bat_dbg_buff + g_file_pos - 4;
        
        buffLen = strlen((const char *)bat_dbg_buff_rptr);
        battery_log(BAT_LOG_CRTI, "battery_dbgbuff_read buffLen=%d\n", buffLen);
        if (buffLen <= count) count = buffLen;

        copy_to_user((void *)buffer, (const void *)bat_dbg_buff_rptr, count);
...
static ssize_t battery_dbgbuff_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    return -EINVAL;
}

static int proc_dbgbuff_show(struct seq_file *m, void *v)
{      
    return 0;
}

static int proc_dbgbuff_open(struct inode *inode, struct file *file)
{
    int ret = 0;
    
    battery_log(BAT_LOG_CRTI, "proc_dbgbuff_open \n");

    ret = single_open(file, proc_dbgbuff_show, NULL);
    g_file_pos = 0;
    return ret;
}

static const struct file_operations battery_dbgbuff_proc_fops = { 
    .open  = proc_dbgbuff_open, 
    .read  = battery_dbgbuff_read,
    .write = battery_dbgbuff_write,
};
#endif  /* TINNO_POWEROFF_CHARGER_DEBUG */
...
static int battery_probe(struct platform_device *dev)
{
    ...
    // For TINNO debug
    {
        struct proc_dir_entry *tinno_dir = NULL;
        
        tinno_dir = proc_mkdir("tinno_dbg", NULL);
        proc_create("dbg_buff", S_IRUGO | S_IWUSR, tinno_dir, &battery_dbgbuff_proc_fops);
        battery_log(BAT_LOG_CRTI, "proc_create battery_dbgbuff_proc_fops\n");
    }
    #endif  /* TINNO_POWEROFF_CHARGER_DEBUG */
