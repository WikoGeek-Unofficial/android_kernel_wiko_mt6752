		...
		bool enable)
{
	...
	if (enable) {
		...
		if (retval < 0) {
			STP_INFO(" Failed to create irq thread\n");
			return retval;
		}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		STP_INFO(" Out of register map limit\n" );
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	STP_INFO(" Attention interrupt disabled\n" );
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	STP_INFO(" Attention interrupt enabled\n" );
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (IS_ERR(dev_data)) {
		STP_INFO(" Pointer of char device data is invalid");
		return -EBADF;
	}
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		STP_INFO(
				" New position 0x%04x is invalid\n",(unsigned int)newpos);
		newpos = -EINVAL;
		goto clean_up;
	}
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		STP_INFO(" rmidev device removed\n" );
	}
...
static int rmidev_create_device_class(void)
{
	...
	if (IS_ERR(rmidev_device_class)) {
		STP_INFO(" Failed to create /dev/%s\n" , CHAR_DEVICE_NAME);
		return -ENODEV;
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmidev) {
		STP_INFO(" Failed to alloc mem for rmidev\n");
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (retval < 0) {
		STP_INFO(" Failed to create device class\n" );
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			STP_INFO(" Failed to allocate char device region\n");
			goto err_device_region;
		}

		rmidev_major_num = MAJOR(dev_no);
		STP_INFO(" Major number of rmidev = %d\n" , rmidev_major_num);
		
	}
	
	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		STP_INFO(" Failed to alloc mem for dev_data\n" );
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		STP_INFO(" Failed to add rmi char device\n" );
	...
	if (IS_ERR(device_ptr)) {
		STP_INFO(" Failed to create rmi char device\n" );
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		STP_INFO(" Failed to export attention gpio\n");
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->board->irq_gpio);
		if (retval < 0) {
			STP_INFO(&rmi4_data->input_dev->dev,"Failed to create gpio symlink\n");
		} else {
			STP_INFO(&rmi4_data->input_dev->dev," Exported attention gpio %d\n", rmi4_data->board->irq_gpio);
		}
	}
#endif
