...
//yaohua.li
//add yaohua.li
extern int APDS9930_get_ps_status(void);
extern struct mutex mutex_power_key;
...
static void synaptics_rmi4_reg_fwu_misc(struct synaptics_rmi4_data *rmi4_data);
					      ...
					      struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		STP_INFO("Failed to issue reset command, error = %d\n", retval);
		return retval;
	}
						 ...
						 struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		STP_INFO(" Failed to read device status, error = %d\n", retval);
		return retval;
	}
...
/*{
    ...
    STP_INFO("I2C write over retry limit\n");
    retval = -EIO;
    }
				   ...
				   unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
	if (!gprDMABuf_va) {
//YAOHUA.LI rmi4_data &rmi4_data->input_dev->dev
//gprDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 4096, &gprDMABuf_pa, GFP_KERNEL);
		...
		if (!gprDMABuf_va) {
			STP_INFO("[Error] Allocate DMA I2C Buffer failed!\n");
		}
	}
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		STP_INFO("I2C read over retry limit\n");
		retval = -EIO;
	}
				    ...
				    unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (!gpwDMABuf_va) {
		//gpwDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 1024, &gpwDMABuf_pa, GFP_KERNEL); //add yaohua.li modify
					 ...
					 struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			{
				tpd_button(x, y, 1);
				STP_INFO("==================!NORMAL_BOOT=========x=%d, y=%d=====boot_mode=%d=============down=======NORMAL_BOOT%d==============\n", x, y, boot_mode, NORMAL_BOOT);
			}
#endif
			STP_INFO(
				 " Finger %d:\n"
	...
	if (touch_count == 0) {
		...
		{
			tpd_button(x, y, 0);
			STP_INFO("==================!NORMAL_BOOT=========x=%d, y=%d=====boot_mode=%d============up========NORMAL_BOOT%d==============\n", x, y, boot_mode, NORMAL_BOOT);
		}
#endif
	}
...
//yaohua.li start
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		{
			STP_INFO("detected_gestures 3\n");
			int ps_status = 0;
			ps_status = APDS9930_get_ps_status();
			STP_INFO("ps_status=%d\n", ps_status);
			if (ps_status)
			{
				STP_INFO("report wakeup event to system.\n");
				gTGesture = 'U';
		...
		} else if (detected_gestures[0] == 0x0B)
		{
			...
			{
				STP_INFO("===synaptics_rmi4_f12_abs_report=====ic%d================\n", detected_gestures[2]);
			case 0x63:
				gTGesture = 'c';
	...
	if (extra_data->data15_size)
	{
		...
		STP_INFO("Number of fingers to process = %d\n", fingers_to_process);
	}
	...
	for (finger = 0; finger < fingers_to_process; finger++)
	{
		...
		if ((!Delicacy_status && finger_status) | (Delicacy_status && finger_Delicacy_status))
		{
			...
			{
				STP_INFO("==================tpd_down==========Delicacy_status====================finger=%d=x%d=y%d====\n", finger, x, y);
			...
			//	STP_INFO("   Finger %d:status = 0x%02x x = %d y = %d wx = %d wy = %d\n" , finger,finger_status,x, y, wx, wy);
			touch_count++;
		}
	}
////////////////////////////////
#ifdef FTM_BUTTON //TPD_HAVE_BUTTON
	if ((FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode) && !finger_status)
	...
	{
		STP_INFO("==================tpd_up==========Delicacy_status=====finger==%d=======x%d=y%d=\n", finger, x, y);
		int ifnum = 0;
				      ...
				      struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		STP_INFO("Failed to read button data registers\n");
		return;
	}
					...
					struct synaptics_rmi4_fn *fhandler)
{
	...
	//STP_INFO("   Function %02x reporting\n" , fhandler->fn_number);
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		//STP_INFO("Failed to read interrupt status\n");
		return;
	}

	status.data[0] = data[0];
	if (status.unconfigured && !status.flash_prog) {
		STP_INFO("spontaneous reset detected\n");
		retval = synaptics_rmi4_reinit_device(rmi4_data);
		if (retval < 0) {
			STP_INFO("Failed to reinit device\n");
		}
		return;
	}
				   ...
				   unsigned int intr_count)
{
	...
	if (!fhandler->extra) {
		STP_INFO("Failed to alloc mem for fhandle->extra\n");
		return -ENOMEM;
	}
	extra_data = (struct synaptics_rmi4_f11_extra_data *)fhandler->extra;
...
#endif
	STP_INFO("Function %02x max x = %d max y = %d\n", fhandler->fn_number, rmi4_data->sensor_max_x,	rmi4_data->sensor_max_y);
				   ...
				   unsigned int intr_count)
{
	...
	fhandler->extra = kmalloc(sizeof(*extra_data), GFP_KERNEL);
	if (!fhandler->extra) {
		STP_INFO("Failed to alloc mem for fhandler->extra\n");
		return -ENOMEM;
	}
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	size_of_2d_data = sizeof(struct synaptics_rmi4_f12_finger_data);
	...
	STP_INFO("[syna]:ctrl_base:%x, ctrl27_offset:%x\n", fhandler->full_addr.ctrl_base, extra_data->ctrl27_offset);
	retval =  synaptics_rmi4_i2c_write(rmi4_data,
					   fhandler->full_addr.ctrl_base + extra_data->ctrl27_offset,
					   &gesture_enable,
					   sizeof(gesture_enable));
	if (retval < 0) {
		STP_INFO("Failed to enable single gesture\n");
		return;
	}
	//yaohua.li end
	...
	if (!fhandler->data) {
		STP_INFO("Failed to alloc mem for fhandler->data\n");
		return -ENOMEM;
	}
					...
					struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		STP_INFO("   Failed to alloc mem for function handle\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		STP_INFO("Failed to read query registers\n");
		return retval;
	}
	...
	if (!f1a->button_control.txrx_map) {
		STP_INFO("   Failed to alloc mem for tx rx mapping\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_data_buffer) {
		STP_INFO("Failed to alloc mem for data buffer\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		STP_INFO(
			 "Failed to alloc mem for button map\n");
		return -ENOMEM;
	}
...
static int synaptics_rmi4_f1a_button_map(struct synaptics_rmi4_data *rmi4_data,
					 struct synaptics_rmi4_fn *fhandler)
{
	STP_INFO("========================synaptics_rmi4_f1a_button_map=============start======================");
	int retval;
	unsigned char ii;
	...
	STP_INFO("========================synaptics_rmi4_f1a_button_map=============end======================");
	return 0;
}
				       ...
				       bool *was_in_bl_mode)
{
	...
	if (status.flash_prog == 1) {
		...
		STP_INFO("   In flash prog mode, status = 0x%02x\n", status.status_code);
	} else {
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
					 ...
					 sizeof(device_ctrl));
	if (retval < 0) {
		STP_INFO("   Failed to set configured\n");
		return;
	}
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (rmi_fd.fn_number == 0) {
				STP_INFO("   Reached end of PDT\n");
				break;
			}

			STP_INFO("   F%02x found (page %d)\n", rmi_fd.fn_number, page_number);

			switch (rmi_fd.fn_number) {
			case SYNAPTICS_RMI4_F01:
				STP_INFO("====LGC====SYNAPTICS_RMI4_F01====\n");
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
								 &rmi_fd, page_number);
				if (retval < 0) {
					STP_INFO("   Failed to alloc for F%d\n", rmi_fd.fn_number);
					return retval;
				}
				...
				if (retval < 0) {
					STP_INFO("   Failed to check status\n");
					return retval;
				}
				...
				break;
			case SYNAPTICS_RMI4_F11:
				STP_INFO("====LGC====SYNAPTICS_RMI4_F11====\n");
				if (rmi_fd.intr_src_count == 0)
					break;
				...
				break;
			case SYNAPTICS_RMI4_F12:
				STP_INFO("====LGC====SYNAPTICS_RMI4_F12====\n");
				if (rmi_fd.intr_src_count == 0)
					break;
				...
				break;
			case SYNAPTICS_RMI4_F1A:
				STP_INFO("====LGC====SYNAPTICS_RMI4_F1A====\n");
				if (rmi_fd.intr_src_count == 0)
					break;
...
flash_prog_mode:
	...
	STP_INFO(
		 "Number of interrupt registers = %d\n", rmi4_data->num_of_intr_regs);
	...
	if (rmi->manufacturer_id != 1) {
		STP_INFO("   Non-Synaptics device found, manufacturer ID = %d\n", rmi->manufacturer_id);
	}
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			STP_INFO("Interrupt enable mask %d = 0x%02x\n", ii, rmi4_data->intr_mask[ii]);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->input_dev == NULL) {
		STP_INFO("   Failed to allocate input device\n");
		retval = -ENOMEM;
	...
	if (retval) {
		STP_INFO("   Failed to register input device\n");
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (Delicacy_status)
	{
		...
		STP_INFO("=====reset=======set_tgesture_unenable=====================\n");
		set_Delicacy_enable();
	}
	return 0;
}
				 ...
				 bool insert)
{
	...
	if (insert) {
		...
		if (!exp_fhandler) {
			STP_INFO("   Failed to alloc mem for expansion function\n");
			goto exit;
		}
...
int sy_open(struct inode *inode, struct file *filp) {
	...
	STP_INFO("[SY]into sy_open\n");
	if (g_pts == NULL)
		STP_INFO("private_ts is NULL~~~");
...
static ssize_t sy_write(struct file *filp, const char *buff, size_t count, loff_t *offp) {
	int ret;
	char *tmp;
	STP_INFO("[SY]into elan_iap_write\n");
	if (count > 8192)
		count = 8192;
...
void synaptics_chip_hw_reset()
{
	...
	msleep(100); //msleep(DELAY_S7300_BOOT_READY);
...
static long sy_ioctl(/*struct inode *inode,*/struct file *filp,    unsigned int cmd, unsigned long arg)
{
	...
	STP_INFO("[ELAN]into elan_iap_ioctl\n");
	STP_INFO("cmd value %x, %ld\n", cmd, arg);
	uint64_t checksum;
	int flag;
	int rc = 0;
	int New_FW_ID = 0;
	...
	rc = mutex_is_locked(&mutex_power_key);
	STP_INFO("mutex state %d\n", rc);
	if (rc)
	...
	case IOCTL_I2C_SY_ID:
		...
		STP_INFO("ioctl IOCTL_I2C_SY_INFO, flag=%d\n", flag);
		if (copy_to_user(ip, &flag, sizeof(int)) != 0) {
			STP_INFO("copy_to_user error");
			rc = -EFAULT;
		}
		break;
	case IOCTL_I2C_SY_IMG_ID:
		flag =  get_synaptics_image_versionid();
		STP_INFO("ioctl IOCTL_I2C_SY_IMG_ID, flag=%d\n", flag);
		if (FACTORY_BOOT != boot_mode)
			mutex_unlock(&mutex_power_key);
	...
	case IOCTL_I2C_SY_UPDATE:
		STP_INFO("ioctl IOCTL_I2C_SY_UPDATE\n");
		mutex_lock(&mutex_power_key);
		flag = 0;
		update_FW();
		synaptics_chip_hw_reset();
		mutex_unlock(&mutex_power_key);
		break;
	default:
		break;
...
static void synaptics_rmi4_reg_fwu_misc(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if(misc_register(&rmi4_data->firmware)< 0)
		STP_INFO("mtk-tpd:[SY] misc_register failed!!\n");
	else
		STP_INFO("[SY] misc_register finished!!\n");
}
...
/*
    mt_set_gpio_mode(GPIO_CTP_EN_PIN,GPIO_CTP_RST_PIN_M_GPIO);// GPIO_CTP_RST_PIN_M_GPIO);
...
*/
	...
	if(!rmi4_data) {
		STP_INFO("   Failed to alloc mem for rmi4_data\n");
		return -ENOMEM;
	}
	...
	if(retval < 0) {
		STP_INFO("Failed to set up input device\n");
		goto err_set_input_dev;
	}

//add by yaohua.li
	synaptics_rmi4_reg_fwu_misc(rmi4_data);
	strcpy(Tg_buf, "mcs");
	SY_key_dev = input_allocate_device();
	if(! SY_key_dev)
	{
		STP_INFO("[STP]SY_key_dev: fail!\n");
	}
	...
	{
		STP_INFO("[STP]SY_key_dev register : fail!\n");
	} else
	{
		STP_INFO("[STP]SY_key_dev register : success!!\n");
	}
//end by yaohua.li
	...
	if(IS_ERR(thread)) {
		...
		STP_INFO("failed to create kernel thread: %d\n", retval);
	}

	retval = synaptics_rmi4_irq_enable(rmi4_data, true);
	if(retval < 0) {
		STP_INFO("Failed to enable attention interrupt\n");
		goto err_enable_irq;
	}
	...
	for(attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if(retval < 0) {
			STP_INFO("Failed to create sysfs attributes\n");
			goto err_sysfs;
		}
	}
				  ...
				  bool enable)
{
	...
	if (retval < 0) {
		STP_INFO("Failed to change reporting mode\n");
		return;
	}
				  ...
				  bool enable)
{
	...
	STP_INFO("=====synaptics_rmi4_f12_wg===start===\n");
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
		if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
		{
			STP_INFO("=======synaptics_rmi4_f12_wg======find====\n");
			break;
		}
	}
...
//yaohua.li
	STP_INFO("=====synaptics_rmi4_f12_wg===end===\n");
	return;
}
...
static void synaptics_rmi4_wakeup_gesture(struct synaptics_rmi4_data *rmi4_data,
					  bool enable)
{
	STP_INFO("====synaptics_rmi4_wakeup_gesture===rmi4_data->f11_wakeup_gesture==%d==rmi4_data->f12_wakeup_gesture%d=\n", rmi4_data->f11_wakeup_gesture, rmi4_data->f12_wakeup_gesture);
	if (rmi4_data->f11_wakeup_gesture)
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
					 ...
					 sizeof(device_ctrl));
	if (retval < 0) {
		STP_INFO("Failed to enter sleep mode\n");
		rmi4_data->sensor_sleep = false;
		return;
	}
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
					 ...
					 sizeof(device_ctrl));
	if (retval < 0) {
		STP_INFO("Failed to wake from sleep mode\n");
		rmi4_data->sensor_sleep = true;
		return;
	}
...
void set_tgesture_enable()
{
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link)
	{
		if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
		{
			STP_INFO("=======set_tgesture_enable======find====\n");
			break;
		}
	}
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
...
void set_tgesture_unenable()
{
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link)
	{
		if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
		{
			STP_INFO("=======et_tgesture_unenable======find====\n");
			break;
		}
	}
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	...
	//yaohua.li end*/
	/*	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	    STP_INFO("==========get_tgesture_unenable==============\n");
	   synaptics_rmi4_wakeup_gesture(rmi4_data, false);*/
	return 0;
}
...
void set_Delicacy_enable()
{
	...
	STP_INFO("==========set__Delicacy_enable==============\n");
	return;
}

void set_Delicacy_unenable()
{
	STP_INFO("==========set__Delicacy_unenable==============\n");
	int retval = 0;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	struct synaptics_rmi4_f12_extra_data *extra_data;
...
void set_sleep_unenable()
{
	...
	STP_INFO("===========synaptics_rmi4_f12_abs_report==set_sleep===================true start====\n");
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	STP_INFO("============synaptics_rmi4_suspend========\n");
	if (rmi4_data->staying_awake)
		return 0;
	mutex_lock(&mutex_power_key);
	STP_INFO("============synaptics_rmi4_suspend===%d====\n", rmi4_data->enable_wakeup_gesture);
	bTPGesturetemp = bEnTGesture;
	STP_INFO("============synaptics_rmi4_suspend==bEnTGesture=%d====\n", bEnTGesture);
	if (bTPGesturetemp && !Delicacy_status) {
		synaptics_chip_hw_reset();
		set_tgesture_enable();
		if (rmi4_data->enable_wakeup_gesture) {
			synaptics_rmi4_wakeup_gesture(rmi4_data, true);
			STP_INFO("============synaptics_rmi4_wakeup_gesture=====\n");
			goto exit;
		}
	} else
	{
		hwPowerDown(TPD_POWER_SOURCE, "TP");
		STP_INFO("============synaptics_rmi4_suspend===set_tgesture_unenable==\n");
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	STP_INFO("============synaptics_rmi4_resume========\n");
	if (rmi4_data->staying_awake)
		return 0;
	mutex_lock(&mutex_power_key);
	STP_INFO("============synaptics_rmi4_resume===%d====\n", rmi4_data->enable_wakeup_gesture);

	if (bTPGesturetemp && !Delicacy_status)
	{
		synaptics_chip_hw_reset();
		/*STP_INFO("============synaptics_chip_hw_reset===");
		msleep(50);
		if (rmi4_data->enable_wakeup_gesture) 
		{		
		STP_INFO("====rmi4_data->enable_wakeup_gesture===%d==\n",rmi4_data->enable_wakeup_gesture);
		synaptics_rmi4_wakeup_gesture(rmi4_data, false);
		synaptics_chip_hw_reset();	*/
		goto exit;
	} else
	{
		synaptics_chip_hw_reset();
		STP_INFO("============synaptics_rmi4_resume====set_tgesture_enable==\n");
	}
	synaptics_rmi4_sensor_wake(rmi4_data);
	synaptics_rmi4_irq_enable(rmi4_data, true);
	retval = synaptics_rmi4_reinit_device(rmi4_data);
...
///////////////////////////////////////////////////////////////////////////////
	...
	{
		set_tgesture_unenable();
		STP_INFO("============set_tgesture_unenable=====================\n");
		set_Delicacy_enable();
		STP_INFO("============set_Delicacy_enable=====================\n");
...
{
	char bufsy[32] = { "SAMSUNG" };
	static char buf[32] = { 0 };
	STP_INFO("===synaptics=======fw==name");
	strcpy(buf, bufsy);
	return buf;
}
...
{
	char bufsy[32] = { "S3508" }; //s3508r"};
	static char buf[32] = { 0 };
	STP_INFO("===synaptics=======IC==name");
	strcpy(buf, bufsy);
	return buf;
}
...
{
	//SAMSUNG-S3508-L5560AE-10
	char tpd_desc_info[50] = { 0 };

#ifdef TINNO_ANDROID_L5560AE
	sprintf(tpd_desc_info, "%s-%s-L5560AE-%d", get_synaptics_fw_name(),
		get_synaptics_IC_name(), get_synaptics_versionid());
#elif defined(TINNO_ANDROID_L8420AE)
	sprintf(tpd_desc_info, "%s-%s-L8420AE-%d", get_synaptics_fw_name(),
		get_synaptics_IC_name(), get_synaptics_versionid());
#endif
	STP_INFO("=======tpd_desc_info=%s===", tpd_desc_info);
...
{
	char tpd_desc_info[50] = { 0 };
	sprintf(tpd_desc_info, "%s-L5560AE-synaptics-0x%x", get_synaptics_fw_name(), get_synaptics_versionid());
	STP_INFO("=======tpd_desc_info=%s===", tpd_desc_info);
...
static struct tpd_driver_t synaptics_rmi4_driver = {
	...
	.suspend = synaptics_rmi4_suspend,
	.resume = synaptics_rmi4_resume,
	.tpd_TpName = get_synaptics_TpName,
	.tpd_TpVersion = get_synaptics_Version,
#ifdef FTM_BUTTON //TPD_HAVE_BUTTON
	.tpd_have_button = 1,
#else
	.tpd_have_button = 0,
...
static int __init synaptics_rmi4_init(void)
{
	...
	if (tpd_driver_add(&synaptics_rmi4_driver) < 0) {
		STP_INFO("Fail to add tpd driver\n");
		return -1;
	}
