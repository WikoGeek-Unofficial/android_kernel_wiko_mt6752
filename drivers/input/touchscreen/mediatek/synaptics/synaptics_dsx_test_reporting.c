...
\
	...
	if (retval < 0) {\
		printk(\
				" Faild to write null terminator\n");\
		return retval;\
	} \
\
	return size + retval;\
...
static void set_report_size(void)
{
	...
	case F54_ADC_RANGE:
		if (f54->query.has_signal_clarity) {
			...
			if (retval < 0) {
				STP_INFO(" Failed to read control reg_41\n");
				f54->report_size = 0;
				break;
			}
...
static int do_preparation(void)
{
	...
	if (f54->query.touch_controller_family == 1) {
		...
		if (retval < 0) {
			STP_INFO(" Failed to disable CBC\n");
			mutex_unlock(&f54->control_mutex);
			return retval;
		}
		...
		if (retval < 0) {
			STP_INFO(" Failed to disable CBC (read ctrl88)\n");
			mutex_unlock(&f54->control_mutex);
			return retval;
		}
		...
		retval = f54->fn_ptr->write(rmi4_data,
				...
				sizeof(f54->control.reg_88->data));
		if (retval < 0) {
			STP_INFO(" Failed to disable CBC (write ctrl88)\n");
			mutex_unlock(&f54->control_mutex);
			return retval;
		}
	}
	...
	if (f54->query.has_0d_acquisition_control) {
		...
		if (retval < 0) {
			STP_INFO(" Failed to disable 0D CBC\n");
			mutex_unlock(&f54->control_mutex);
			return retval;
		}
	}
	...
	if (f54->query.has_signal_clarity) {
		...
		retval = f54->fn_ptr->write(rmi4_data,
				...
				sizeof(f54->control.reg_41->data));
		if (retval < 0) {
			STP_INFO(" Failed to disable signal clarity\n");
			mutex_unlock(&f54->control_mutex);
			return retval;
		}
	}
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		STP_INFO(" Failed to write force update command\n");
		return retval;
	}
	...
	do {
		retval = f54->fn_ptr->read(rmi4_data,
				...
				sizeof(value));
		if (retval < 0) {
			STP_INFO(" Failed to read command register\n");
			return retval;
		}
	...
	if (timeout_count == FORCE_TIMEOUT_100MS) {
		STP_INFO(" Timed out waiting for force update\n");
		return -ETIMEDOUT;
	}
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		STP_INFO(" Failed to write force cal command\n");
		return retval;
	}
	...
	if (timeout_count == FORCE_TIMEOUT_100MS) {
		STP_INFO(" Timed out waiting for force cal\n");
		return -ETIMEDOUT;
	}
...
static void timeout_set_status(struct work_struct *work)
{
	...
	if (f54->status == STATUS_BUSY) {
		...
		if (retval < 0) {
			...
			STP_INFO(" Report type not supported by FW\n");
		} else {
...
static void print_image_report(void)
{
	...
	case F54_FULL_RAW_CAP_RX_COUPLING_COMP:
		...
		STP_INFO(" End of report\n");
		break;
	default:
		STP_INFO("Image not supported for report type %d\n",f54->report_type);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		STP_INFO(" Failed to read control register\n");
		return retval;
	}
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		STP_INFO(" Failed to write control register\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!is_report_type_valid((enum f54_report_types)setting)) {
		STP_INFO(" Report type not supported by driver\n");
		return -EINVAL;
	}
	...
	if (f54->status != STATUS_BUSY) {
		...
		mutex_unlock(&f54->status_mutex);
		if (retval < 0) {
			STP_INFO(" Failed to write data register\n");
			return retval;
		}
		return count;
	} else {
		STP_INFO(" Previous get report still ongoing\n");
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		STP_INFO(" Failed to read data registers\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		STP_INFO(" Failed to write data registers\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		STP_INFO(" Failed to do preparation\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_unlock(&f54->status_mutex);
	if (retval < 0) {
		STP_INFO(" Failed to write get report command\n");
		return retval;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		STP_INFO(" Failed to read control reg_17\n" );
	}
	...
	if (retval < 0) {
		STP_INFO(" Failed to read control reg_18\n" );
	}
	...
	for (ii = 0; ii < f54->control.reg_17->length; ii++) {
		...
		if (retval < 0) {
			STP_INFO(" Faild to write output\n" );
			return retval;
		}
		size += retval;
		temp += retval;
	...
	retval = snprintf(temp, PAGE_SIZE - size, "\n");
	if (retval < 0) {
		STP_INFO(" Faild to write null terminator\n" );
		return retval;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < f54->report_size) {
		STP_INFO(" Report type %d data size (%d) too large\n" , f54->report_type, f54->report_size);
	...
	if (f54->report_data) {
		...
		STP_INFO(" Report type %d data not available\n" , f54->report_type);
		mutex_unlock(&f54->data_mutex);
		return -EINVAL;
	}
}
...
static int synaptics_rmi4_f54_set_sysfs(void)
{
	...
	f54->attr_dir = kobject_create_and_add("f54",&rmi4_data->input_dev->dev.kobj);
	if (!f54->attr_dir) {
		STP_INFO(" Failed to create sysfs directory\n" );
		goto exit_1;
	}
	...
	for (reg_num = 0; reg_num < ARRAY_SIZE(attrs_ctrl_regs); reg_num++) {
		if (attrs_ctrl_regs_exist[reg_num]) {
			retval = sysfs_create_group(f54->attr_dir,&attrs_ctrl_regs[reg_num]);
			if (retval < 0) {
				STP_INFO(" Failed to create sysfs attributes\n" );
				goto exit_4;
			}
...
exit_no_mem:
	STP_INFO(" Failed to alloc mem for control registers\n");
	return -ENOMEM;
}
...
static void synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(report_index));
	if (retval < 0) {
		STP_INFO(" Failed to write report data index\n");
		retval = -EINVAL;
		goto error_exit;
	}
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			f54->report_size);
	if (retval < 0) {
		STP_INFO(" Failed to read report data\n");
		retval = -EINVAL;
		goto error_exit;
	}
...
static void synaptics_rmi5_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		STP_INFO(" Failed to read f55 query registers\n");
		return;
	}
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			rx_electrodes);
	if (retval < 0) {
		STP_INFO(" Failed to read f55 rx assignment\n");
		return;
	}
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			tx_electrodes);
	if (retval < 0) {
		STP_INFO(" Failed to read f55 tx assignment\n");
		return;
	}
		...
		unsigned char page)
{
	...
	if (!f55) {
		STP_INFO(" Failed to alloc mem for f55\n");
		return;
	}
...
static int synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!f54) {
		STP_INFO(" Failed to alloc mem for f54\n");
		retval = -ENOMEM;
		goto exit;
	}
...
pdt_done:
	...
	if (retval < 0) {
		STP_INFO(" Failed to read f54 query registers\n");
		goto exit_free_mem;
	}
	...
	if (retval < 0) {
		STP_INFO(" Failed to set up f54 control registers\n");
	...
	if (retval < 0) {
		STP_INFO(" Failed to create sysfs entries\n");
		goto exit_sysfs;
	}
