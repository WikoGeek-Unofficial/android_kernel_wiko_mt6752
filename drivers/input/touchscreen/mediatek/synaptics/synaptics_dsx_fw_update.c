...
static ssize_t fwu_sysfs_disp_config_block_count_show(struct device *dev,
		struct device_attribute *attr, char *buf);
int fwu_IMG_ID();
void fwu_startup_fw_update_work(struct work_struct *work);
enum bl_version {
	V5 = 5,
	V6 = 6,
};
...
static int fwu_read_f01_device_status(struct f01_device_status *status)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(status->data));
	if (retval < 0) {
		STP_INFO("Failed to read F01 device status\n");
		return retval;
	}
...
static int fwu_read_f34_queries(void)
{
	...
	if (retval < 0) {
		STP_INFO("Failed to read bootloader ID\n");
		return retval;
	}
	...
	if (fwu->bootloader_id[1] == '5') {
		...
		STP_INFO("Unrecognized bootloader version\n");
		return -EINVAL;
	}
	...
	if (retval < 0) {
		STP_INFO("Failed to read flash properties\n");
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			2);
	if (retval < 0) {
		STP_INFO("Failed to read block size info\n");
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			count);
	if (retval < 0) {
		STP_INFO(" Failed to read block count info\n");
		return retval;
	}
...
static int fwu_read_f34_flash_status(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(status));
	if (retval < 0) {
		STP_INFO(" Failed to read flash status\n");
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		STP_INFO("Failed to read flash command\n");
		return retval;
	}
...
int get_synaptics_versionid( void )
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				config_id,
				sizeof(config_id));
	 STP_INFO("==========config ID%d:\n",config_id[3]);
	if (retval < 0) 
	{
	    STP_INFO("Failed to read device config ID\n");
            return 0;
	}
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	if (header->contains_firmware_id) {
		...
		if (!strptr) {
			STP_INFO("No valid PR number (PRxxxxxxx)found in image file name (%s)\n",fwu->image_name);
			flash_area = NONE;
			goto exit;
		}
		...
		kfree(firmware_id);
		if (retval) {
			STP_INFO(" Failed to obtain image firmware ID\n");
			flash_area = NONE;
			goto exit;
		}
	}
	STP_INFO("Image firmware ID = %d\n",(unsigned int)image_fw_id);

	if (image_fw_id > device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else if (image_fw_id < device_fw_id) {
		STP_INFO("Image firmware ID older than device firmware ID\n");
		flash_area = NONE;
		goto exit;
	}
	...
	STP_INFO("Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
			config_id[0],
			config_id[1],
			config_id[2],
			config_id[3]);
	...
	STP_INFO(" Image config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
			fwu->config_data[3]);
...
exit:
	if (flash_area == NONE) {
		STP_INFO("No need to do reflash\n");
	} else {
...
static int fwu_scan_pdt(void)
{
	...
	if (!f01found || !f34found) {
		STP_INFO("Failed to find both F01 and F34\n");
		return -EINVAL;
	}
		...
		unsigned char command)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(block_offset));
	if (retval < 0) {
		STP_INFO(" Failed to write to block number registers\n");
		return retval;
	}
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		retval = fwu->fn_ptr->write(fwu->rmi4_data,
				...
				fwu->block_size);
		if (retval < 0) {
			STP_INFO("Failed to write block data (block %d)\n",block_num);
			return retval;
		}

		retval = fwu_write_f34_command(command);
		if (retval < 0) {
			STP_INFO("Failed to write command for block %d\n", block_num);
			return retval;
		}
...
static int fwu_write_bootloader_id(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(fwu->bootloader_id));
	if (retval < 0) {
		STP_INFO("Failed to write bootloader ID\n");
		return retval;
	}
...
static int fwu_enter_flash_prog(void)
{
	...
	if (!fwu->program_enabled) {
		STP_INFO("Program enabled bit not set\n");
		return -EINVAL;
	}
	...
	if (!f01_device_status.flash_prog) {
		STP_INFO("Not in flash prog mode\n");
		return -EINVAL;
	}
	...
	if (retval < 0) {
		STP_INFO("Failed to read F01 device control\n");
		return retval;
	}
...
static int fwu_do_reflash(void)
{
	...
	STP_INFO("Entered flash prog mode\n");

	retval = fwu_write_bootloader_id();
	if (retval < 0)
		return retval;

	STP_INFO("Bootloader ID written\n");

	retval = fwu_write_f34_command(CMD_ERASE_ALL);
	if (retval < 0)
		return retval;

	STP_INFO("Erase all command written\n");

	retval = fwu_wait_for_idle(ERASE_WAIT_MS);
	if (retval < 0)
		return retval;

	STP_INFO("Idle status detected\n");

	if (fwu->firmware_data) {
		retval = fwu_write_firmware();
		if (retval < 0)
			return retval;
		STP_INFO("Firmware programmed\n");
	}
	...
	if (fwu->config_data) {
		...
		if (retval < 0)
			return retval;
		STP_INFO("Configuration programmed\n");
	}
...
static int fwu_do_write_config(void)
{
	...
	STP_INFO("Erase command written\n");
...
static int fwu_start_write_config(void)
{
	...
	STP_INFO("Start of write config process\n");

	retval = fwu_do_write_config();
	if (retval < 0) {
		STP_INFO(" Failed to write config\n");
	}

	fwu->rmi4_data->reset_device(fwu->rmi4_data);

	STP_INFO("End of write config process\n");
...
static int fwu_do_read_config(void)
{
	...
	for (block_num = 0; block_num < block_count; block_num++) {
		...
		if (retval < 0) {
			STP_INFO("Failed to write read config command\n");
			goto exit;
		}

		retval = fwu_wait_for_idle(WRITE_WAIT_MS);
		if (retval < 0) {
			STP_INFO("Failed to wait for idle status\n");
			goto exit;
		}
		...
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				fwu->block_size);
		if (retval < 0) {
			STP_INFO("Failed to read block data (block %d)\n",block_num);
			goto exit;
		}
...
static int fwu_do_lockdown(void)
{
	...
	if ((fwu->flash_properties & UNLOCKED) == 0) {
		STP_INFO("Device already locked down\n");
		return retval;
	}
...
int fwu_IMG_ID(void)
{
	...
	if (fwu->rmi4_data->sensor_sleep) {
		STP_INFO("Sensor sleeping\n");
		return -ENODEV;
	}

	fwu->rmi4_data->stay_awake = true;

	STP_INFO("Start of reflash process\n");

	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
	} else {
		strncpy(fwu->image_name, FW_IMAGE_NAME, MAX_IMAGE_NAME_LEN);
		STP_INFO("Requesting firmware image %s\n", fwu->image_name);
		...
		STP_INFO("Firmware image size = %d\n",fw_entry->size);
	...
	if (fwu->bl_version != header.bootloader_version) {
		STP_INFO("Bootloader version mismatch\n");
		retval = -EINVAL;
		goto exit;
	}
	...
	if (fwu->do_lockdown) {
		...
		case V6:
			...
			if (retval < 0) {
				STP_INFO("Failed to do lockdown\n");
			}
		default:
			break;
		}
	...
	if (header.config_size) {
        ...
        STP_INFO("==== fwu_IMG_ID=%d====", extract_uint_be(fwu->config_data));
...
exit:
	...
	STP_INFO("End of reflash process\n");
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		retval = request_firmware(&fw_entry, fwu->image_name,
				&fwu->rmi4_data->i2c_client->dev);
		if (retval != 0) {
			STP_INFO("Firmware image %s not available\n",fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		STP_INFO("Not enough space (%d bytes) in buffer\n",count);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		STP_INFO("Failed to read config\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu->ext_data_source) {
		STP_INFO("Failed to alloc mem for image data\n");
		return -ENOMEM;
	}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!fwu) {
		STP_INFO(" Failed to alloc mem for fwu\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!fwu->fn_ptr) {
		STP_INFO("Failed to alloc mem for fn_ptr\n");
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (!fwu->image_name) {
		STP_INFO("Failed to alloc mem for image name\n");
		retval = -ENOMEM;
	...
	if (retval < 0) {
		STP_INFO("Failed to read PDT properties, assuming 0x00\n");
	} else if (pdt_props.has_bsr) {
		STP_INFO(" Reflash for LTS not currently supported\n");
		retval = -ENODEV;
		goto exit_free_mem;
	}
	...
	STP_INFO(": F01 product info: 0x%04x 0x%04x\n",fwu->productinfo1, fwu->productinfo2);
	STP_INFO("F01 product ID: %s\n",fwu->product_id);
	...
	retval = sysfs_create_bin_file(&rmi4_data->input_dev->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
		STP_INFO("Failed to create sysfs bin file\n");
		goto exit_free_mem;
	}
