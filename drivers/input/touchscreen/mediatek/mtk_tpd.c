...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name == NULL) {
			...
			tpd_driver_list[i].tpd_TpVersion = tpd_drv->tpd_TpVersion;
...
{
	tpd_fw_version =   g_tpd_drv->tpd_TpVersion();
#ifndef TINNO_ANDROID_S7812BP
	tpd_fw_version =   g_tpd_drv->tpd_TpVersion();
...
/*#if defined(TINNO_ANDROID_S3520AP) || defined(TINNO_ANDROID_S3521AP)
	sprintf(tpd_desc, "%s",g_tpd_drv->tpd_TpName());
#elif defined TINNO_ANDROID_S7812BP
    sprintf(tpd_desc, "%s-S7812BP-%s-%X",get_fw_name(),"FT5436", get_fw_version_ext());
