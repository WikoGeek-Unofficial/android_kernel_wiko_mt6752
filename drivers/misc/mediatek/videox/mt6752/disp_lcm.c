...
int disp_lcm_init(disp_lcm_handle *plcm, int force)
{
	...
	if(_is_lcm_inited(plcm))
	{
		...
		// [JSTINNO_SRC xiaoyan.yu,  add for supporting ea8061 can successfully recover form esd fail, DATE20141209-01 START
		#ifdef EA8061V_HD720_DSI_VIDEO_AMOLED
		if(lcm_drv->resume != NULL && (strcmp(lcm_drv->name, "ea8061v_dsi_vdo_lcm_drv_amoled")==0) && force)
		lcm_drv->resume();
		else
		#endif/*EA8061V_HD720_DSI_VIDEO_AMOLED*/
		// JSTINNO_SRC xiaoyan.yu, DATE20141209-01 END]
        {
		if(lcm_drv->init)
		{
			if(!disp_lcm_is_inited(plcm) || force)
