...
#include <linux/proc_fs.h>	//JSTINNO_SRC xiaoyan.yu,  add misc device for refering lcm name in EM ,DATE20140829-01 LINE
...
#ifdef EA8061V_HD720_DSI_VIDEO_AMOLED
...
int primary_display_esd_recovery(void)
{
	...
	#ifdef EA8061V_HD720_DSI_VIDEO_AMOLED
	{
         disp_lcm_set_backlight(pgc->plcm,g_backlight);
...
long lcm_panel_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	DISPCHECK("lcm_panel_ioctl cmd=%d\n",cmd);
	...
	switch(cmd)
	{
		...
		{
			int set_dim = 0;
			if(copy_from_user(&set_dim, ptr, sizeof(set_dim)))
			{
				err = -EFAULT;
				goto err_out;
			}
			...
			if(pgc->plcm->drv->change_backligth_to_dim_mode != NULL)
			{
			   DISPCHECK("lcm_panel_ioctl change_backligth_to_dim_mode set_dim=%d\n",set_dim);
			   pgc->plcm->drv->change_backligth_to_dim_mode(set_dim);
			}
			_primary_path_unlock(__func__);
			_primary_path_cmd_unlock();
		...
		default:
		{
			DISPERR("[lcm_panel]ioctl not supported, 0x%08x\n", cmd);
		}
	}
...
static const struct file_operations lcminfo_proc_fops = { 
    .read       = lcm_panel_read,
};
		...
		dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);//this should remove? for video mode when LK has start path
         ...
         #if 1
         {
...

            {
                 ...
                 struct proc_dir_entry *lcminfo_proc;
                 lcminfo_proc = proc_create("lcm_info", 0444, NULL, &lcminfo_proc_fops);
                 if (!lcminfo_proc) {
                  printk(KERN_ERR "lfail to register /proc/lcm_info\n");
                 }
            }
...
int primary_display_suspend(void)
{
	...
	#ifdef EA8061V_HD720_DSI_VIDEO_AMOLED
	if(pgc->plcm->drv->goto_suspend != NULL)
	    pgc->plcm->drv->goto_suspend();
	#endif/*EA8061V_HD720_DSI_VIDEO_AMOLED*/
