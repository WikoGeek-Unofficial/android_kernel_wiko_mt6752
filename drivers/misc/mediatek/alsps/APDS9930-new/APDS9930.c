...
//#include "../../../../../../../include/linux/sensors_io.h"
/******************************************************************************
 * configuration
*******************************************************************************/
/*----------------------------------------------------------------------------*/
...
int  g_bsunps =0;//0 is normal sun 1 is out door sun
...
int APDS9930_get_ps_status(void);
/*----------------------------------------------------------------------------*/
...
//yaohua.li start
int APDS9930_CMM_CONTROL_VALUE_S=0XEE;
int APDS9930_CMM_PPCOUNT_VALUE_S = 0X8;
//yaohua.li end
...
static int APDS9930_i2c_master_operate(struct i2c_client *client, const char *buf, int count, int i2c_flag);
...
static void APDS9930_ps_cali_start(struct APDS9930_priv *obj)
{
    ...
    printk("entry ap3220_ps_cali_start \n");
    
    if(NULL == obj->client)
    {
        printk("obj->client == NULL\n"); 
        return;
    }
    ...
    g_ps_cali_flag = 1;    
    printk("ap3220_ps_cali_start:g_ps_base_value=%x \n",g_ps_base_value);
    
exit_handle:
	printk("tag: exit_handle\n");
...
static void APDS9930_ps_cali_set_threshold(struct APDS9930_priv *obj)
{
    ...
    printk("APDS9930_ps_cali_set_threshold:g_ps_base_value=%x, hw->ps_threshold_high=%x,hw->ps_threshold_low=%x \n",
    ...
    {
	value_ps_high = g_ps_base_value + 0X200;
	value_ps_low  = g_ps_base_value + 0X180;
    }
    else if( g_ps_base_value < 0x180)
    {
        value_ps_high = g_ps_base_value + 0X270;
	value_ps_low  = g_ps_base_value + 0X230;
    }
    if(value_ps_high >0X300)
   {
     value_ps_high = 0X310;
     value_ps_low  = 0X380;
	u8 databuf[2]; 
	/*modify  APDS9930_CMM_CONTROL_VALUE */
	databuf[0] = APDS9930_CMM_CONTROL;    
	databuf[1] = APDS9930_CMM_CONTROL_VALUE_S;
	res = APDS9930_i2c_master_operate(obj->client, databuf, 0x2, I2C_FLAG_WRITE);
	if(res <= 0)
	{
	  	printk("=========================add APDS9930=========================error=========\n");
	}
   }
	...
	printk("set_psensor_threshold function high: 0x%x, low:0x%x\n",atomic_read(&obj->ps_thd_val_high),atomic_read(&obj->ps_thd_val_low));
	...
	printk("APDS9930_ps_cali_set_threshold:value_ps_high=%x,value_ps_low=%x! \n",value_ps_high,value_ps_low);
}
...
static int APDS9930_i2c_master_operate(struct i2c_client *client, const char *buf, int count, int i2c_flag)
{
	...
	break;
	default:
	printk("APDS9930_i2c_master_operate i2c_flag command not support!\n");
	break;
	}
	if(res <= 0)
	{
	...
	EXIT_ERR:
	mutex_unlock(&APDS9930_mutex);
	printk("APDS9930_i2c_transfer fail\n");
	return res;
}
...
static long APDS9930_enable_als(struct i2c_client *client, int enable)
{
	...
	//printk("APDS9930_CMM_ENABLE als value = %x\n",databuf[0]);
	
	if(enable)
		{
			databuf[1] = databuf[0]|0x03;
			databuf[0] = APDS9930_CMM_ENABLE;
			//printk("APDS9930_CMM_ENABLE enable als value = %x\n",databuf[1]);
			res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
		...
		if(test_bit(CMC_BIT_PS, &obj->enable))
			...
			databuf[0] = APDS9930_CMM_ENABLE;
			//printk("APDS9930_CMM_ENABLE disable als value = %x\n",databuf[1]);
			res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
...
EXIT_ERR:
	printk("APDS9930_enable_als fail\n");
	return res;
}
...
static long APDS9930_enable_ps(struct i2c_client *client, int enable)
{
	...
	//printk("APDS9930_CMM_ENABLE ps value = %x\n",databuf[0]);
	
	if(enable)
		{
			databuf[1] = databuf[0]|0x05;
			databuf[0] = APDS9930_CMM_ENABLE;
			//printk("APDS9930_CMM_ENABLE enable ps value = %x\n",databuf[1]);	
			res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
			}
			if(0 != obj->hw->polling_mode_ps) /*polling mode*/
			{
			...
			if(0 == obj->hw->polling_mode_ps) /*interrupt mode*/
			{
		...
		if(test_bit(CMC_BIT_ALS, &obj->enable))
			...
			databuf[0] = APDS9930_CMM_ENABLE;
			//printk("APDS9930_CMM_ENABLE disable ps value = %x\n",databuf[1]);	
			res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
...
EXIT_ERR:
	printk("APDS9930_enable_ps fail\n");
	return res;
}
/*----------------------------------------------------------------------------*/
...
EXIT_ERR:
	printk("APDS9930_check_and_clear_intr fail\n");
	return 1;
}
/*----------------------------------------------------------------------------*/
...
EXIT_ERR:
	printk("APDS9930_check_intr fail\n");
	return 1;
}
...
static int LuxCalculation(struct i2c_client *client, int ch0data, int ch1data)
{
	...
	//printk("IAC1=%d, IAC2=%d, IAC=%d ---CYY\n", IAC1, IAC2, IAC);
...
static ssize_t APDS9930_store_tp_color(struct device_driver *ddri, char *buf, size_t count)
{
	...
	printk("Entry APDS9930_store_tp_color buf string=%s\n", buf);
...
{	 
	...
	/*if(tp_color_val == 2)
		printk("White Panel ---CYY\n");
	else
		printk("BLack Panel ---CYY\n");
	*/
	...
	printk("c0_value=%d, c0_nf=%d, als_modulus=%d ---CYY\n", c0_value, c0_nf, obj->als_modulus);
	...
	printk("c1_value=%d, c1_nf=%d, als_modulus=%d ---CYY\n", c1_value, c1_nf, obj->als_modulus);
	...
	{
		printk("APDS9930_read_als als_value is invalid!!\n");
		return -1;
	}	
	#endif

	//printk("ch0data=%d, ch1data=%d ---CYY\n", ch0data, ch1data);
	...
	*data = LuxCalculation(client, ch0data, ch1data);
	//printk("APDS9930_read_als als_value_lux = %d  ---CYY\n", *data);
	return 0;	 
	
	
	
EXIT_ERR:
	printk("APDS9930_read_ps fail\n");
	return res;
}
...
static int APDS9930_get_als_value(struct APDS9930_priv *obj, u16 als)
{
	...
	{
		printk("APDS9930_get_als_value exceed range\n"); 
...
long APDS9930_read_ps(struct i2c_client *client, u16 *data)
{
	...
	//printk("yucong APDS9930_read_ps ps_data=%d, low:%d  high:%d", *data, buffer[0], buffer[1]);
...
static int APDS9930_get_ps_value(struct APDS9930_priv *obj, u16 ps)
{
	...
	if(ps_cali.valid == 1)
    {
        printk("==APDS9930_get_ps_value g_bsunps  = %d,ps==%d,==atomic_read(&obj->ps_thd_val_high)==%d\n",g_bsunps,ps,ps_cali.close);
	if(2==g_bsunps)
	{
	    val = 1;  /*far away*/
            val_temp = 1;
	...
	else
    {
        ...
        printk("==APDS9930_get_ps_value g_bsunps  = %d,ps==%d,==atomic_read(&obj->ps_thd_val_high)==%d\n",g_bsunps,ps,atomic_read(&obj->ps_thd_val_high));
	if(2==g_bsunps)
	{
	    val = 1;  /*far away*/
            val_temp = 1;
        ...
        else if((ps  < atomic_read(&obj->ps_thd_val_low)))
        {
            ...
            val = val_temp;	
        printk("APDS9930_get_ps_value value_ps_high==%d\n",value_ps_high);
        printk("APDS9930_get_ps_value value_ps_low==%d\n",value_ps_low);
        atomic_set(&obj->ps_thd_val_high, value_ps_high);
        atomic_set(&obj->ps_thd_val_low,  value_ps_low);
    }
	...
	{
		//invalid = 1;              
		printk("ligh too high will result to failt proximiy\n");
		return 1;  //far away
	}*/
...
static void APDS9930_eint_work(struct work_struct *work)
{
	...
	{
		printk("APDS9930_eint_work check intrs: %d\n", err);
	}
	else
	{
		//get raw data
		...
		//printk("APDS9930 int top half time = %lld\n", int_top_time);
	//	printk("APDS9930_eint_work rawdata ps=%d als_ch0=%x!==als_ch0>0x1300=%d==(obj->als>0x1000)=%d\n",obj->ps,obj->als,(obj->als>0x1300),(obj->als>0x1000));
		if(obj->als > 0x1800)
		{
			g_bsunps = 2;
			//printk("======APDS9930_eint_work ALS too large may under lighting als_ch0=%x!.g_bsunps=%d\n",obj->als,g_bsunps);
			//return ;
		}
                else if(obj->als > 0x1000)
		{
                        g_bsunps = 2;
			//printk("XXXXXXXXX====APDS9930_eint_work ALS too large may under lighting als_ch0=%d!.g_bsunps=%d\n",obj->als,g_bsunps);
		...
		printk("APDS9930_eint_work APDS9930_CMM_ENABLE ps value = %x\n",databuf[0]);
		...
		printk("APDS9930_eint_work APDS9930_CMM_INT_LOW_THD_LOW before databuf[0]=%d databuf[1]=%d!\n",databuf[0],databuf[1]);
		...
		printk("APDS9930_eint_work APDS9930_CMM_INT_HIGH_THD_LOW before databuf[0]=%d databuf[1]=%d!\n",databuf[0],databuf[1]);
#endif
...
/*singal interrupt function add*/
		...
		printk("APDS9930_eint_work APDS9930_CMM_INT_LOW_THD_LOW after databuf[0]=%d databuf[1]=%d!\n",databuf[0],databuf[1]);
		...
		printk("APDS9930_eint_work APDS9930_CMM_INT_HIGH_THD_LOW after databuf[0]=%d databuf[1]=%d!\n",databuf[0],databuf[1]);
		#endif
...
static long APDS9930_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (!file->f_op || !file->f_op->unlocked_ioctl)
		return -ENOTTY;
    printk("APDS9930_compat_ioctl arg: 0x%lx, arg32: 0x%p\n",arg, arg32);
	switch (cmd) {			 
		 case COMPAT_ALSPS_IOCTL_PS_SET_CALI:
			 if(arg32 == NULL)
			 {
				 printk("invalid argument.");
				 return -EINVAL;
			 }
			 ret = file->f_op->unlocked_ioctl(file, ALSPS_IOCTL_PS_SET_CALI ,
							(unsigned long)(arg32));
			 if (ret){
			 	printk("ALS_PS SENSOR_IOCTL_READ_FACTORY_SENSORDATA unlocked_ioctl failed.");
				return ret;
			 }	
			 break;
	 case COMPAT_ALSPS_GET_PS_RAW_DATA:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, ALSPS_GET_PS_RAW_DATA ,
							(unsigned long)(arg32));
			 if (ret){
			 	printk("ALSPS_PSENSOR_IOCTL_READ_FACTORY_SENSORDATA unlocked_ioctl failed.");
				return ret;
			 }	
			 break;
		 default:
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_TEST_RESULT:
			...
			case ALSPS_IOCTL_PS_SET_CALI:
				...
				printk("not calibration!!! set g_ps_base_value = 0x80 \n");
				}
			...
			case ALSPS_SET_PS_THRESHOLD:
				...
				printk("%s set threshold high: 0x%x, low: 0x%x\n", __func__, threshold[0],threshold[1]); 
			...
			case ALSPS_GET_PS_THRESHOLD_HIGH:
				...
				printk("%s get threshold high: 0x%x\n", __func__, threshold[0]); 
			...
			case ALSPS_GET_PS_THRESHOLD_LOW:
				...
				printk("%s get threshold low: 0x%x\n", __func__, threshold[0]); 
				if(copy_to_user(ptr, &threshold[0], sizeof(threshold[0])))
				{
					err = -EFAULT;
					goto err_out;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				APDS9930_read_ps(obj->client, &obj->ps);
				APDS9930_read_als_ch0(obj->client, &obj->als);
				printk("APDS9930_ps_operate ps data=%d!\n",obj->ps);
				printk("APDS9930_ps_operate als data=%d!\n",obj->als);
				sensor_data->values[0] = APDS9930_get_ps_value(obj, obj->ps);
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;			
			}
...
//#if defined (TINNO_TGESTURE_BY_PS)
int APDS9930_get_ps_status(void )
{
    int ps_status = 0;
	int err = 0;
    int needRestore = 0;
    int count = 20; // 100 ms
    struct APDS9930_priv *obj = (struct APDS9930_priv *)APDS9930_obj;
    ...
    do 
    {
        ...
        buffer[0] = APDS9930_CMM_STATUS;
        APDS9930_i2c_master_operate(obj->client, buffer, 0x101, I2C_FLAG_READ);
        //printk("APDS9930_CMM_STATUS 0x%x, count=%d\n", buffer[0], count);

        if (buffer[0] & 0x02)   // Check PVALID
            break;
    ...
    //printk("ps data=%d\n",obj->ps);
    //printk("als data=%d\n",obj->als);
    ps_status = APDS9930_get_ps_value(obj, obj->ps);	
    ...
    printk("ps_status %d, needRestore=%d\n", ps_status, needRestore);

    return ps_status;
}
//#endif  /* TINNO_TGESTURE_BY_PS */
...
static int APDS9930_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("APDS9930_init_client() OK!\n");

	if((err = misc_register(&APDS9930_device)))
	{
		printk("APDS9930_device register failed\n");
...
static int APDS9930_i2c_remove(struct i2c_client *client)
{
	...
	{
		printk("APDS9930_delete_attr fail: %d\n", err);
	} 
