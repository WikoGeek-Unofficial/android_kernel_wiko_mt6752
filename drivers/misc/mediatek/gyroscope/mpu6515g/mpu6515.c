...
int mpu6515_gyro_init_client(bool enable);
...
extern int mpu6515_init_client(int reset_cali);
extern int MPU6515_Dev_Reset(void);
#endif 
...
static DEFINE_MUTEX(gyro_resume_mutex);
static DEFINE_MUTEX(gyro_init_mutex);

static u8 g_gyr_last_data[128] = {0};
...
while(atomic_read(&obj->suspend) && loop-->0)
{
	printk("wanghai, MPU6515_ReadFifoData will sleep a moment\n");
	udelay(100);
}//wanghai
...
static int MPU6515_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
   ...
   if(bufsize<=0) {
	printk("wanghai, MPU6515_ReadGyroData, bufsize <=0, return\n");
	return -1;
   }

     memcpy((void*)buf, (void*)g_gyr_last_data, 
          sizeof(g_gyr_last_data)<bufsize?sizeof(g_gyr_last_data):bufsize);

	mutex_lock(&gyro_resume_mutex);
	if((atomic_read(&obj->suspend) == 0) && atomic_read(&g_after_resume) > 0)
	{
		atomic_dec(&g_after_resume);
		mutex_unlock(&gyro_resume_mutex);
		MPU6515_Dev_Reset();
		mpu6515_init_client(false);
		mpu6515_gyro_init_client(false);
        //printk("wanghai, MPU6515_ReadGyroData, ==0 >0, g_after_resume: %\n", g_after_resume);
	}else if (atomic_read(&g_after_resume) > 0){    
		mutex_unlock(&gyro_resume_mutex);
		printk("wanghai, MPU6515_ReadGyroData return, in suspend\n");
		return -1;
	}else{
		mutex_unlock(&gyro_resume_mutex);
        printk("wanghai, MPU6515_ReadGyroData enter\n");
	}
    ...
    sprintf(buf, "%04x %04x %04x", data[MPU6515_AXIS_X],data[MPU6515_AXIS_Y],data[MPU6515_AXIS_Z]);
     ...
     memcpy((void*)g_gyr_last_data, (void*)buf, 
             sizeof(g_gyr_last_data)<bufsize?sizeof(g_gyr_last_data):bufsize);
...
int mpu6515_gyro_init_client(bool enable)
{
    ...
    /*if(mutex_trylock(&gyro_init_mutex)){     
        printk("wanghai, another is init gyro\n");
        return 0;  
    } */
    
    mutex_lock(&gyro_init_mutex);   

    if(mpu6515_i2c_client == NULL){
        res = -1;
	    goto ERR_RET;
    }else{
        obj = i2c_get_clientdata(mpu6515_i2c_client); 
    }

    GYRO_FUN(); 
    mpu6515_gpio_config();

    res = MPU6515_SetPowerMode(mpu6515_i2c_client, true);
    ...
    res = MPU6515_SetDataFormat(mpu6515_i2c_client, (MPU6515_SYNC_GYROX << MPU6515_EXT_SYNC)|
                                MPU6515_RATE_1K_LPFB_188HZ);

    res = MPU6515_SetFullScale(mpu6515_i2c_client, (MPU6515_DEFAULT_FS << MPU6515_FS_RANGE));
    if (res != MPU6515_SUCCESS)
    {
        goto ERR_RET;
    }
    ...
    res = MPU6515_SetSampleRate(mpu6515_i2c_client, 125);
    if (res != MPU6515_SUCCESS )
    {
       goto ERR_RET;
    }

    res = MPU6515_SetPWR_MGMT_2(mpu6515_i2c_client, enable_status);
    if (res != MPU6515_SUCCESS)
    {
        goto ERR_RET;
    }

    res = MPU6515_SetPowerMode(mpu6515_i2c_client, enable_status);
    if (res != MPU6515_SUCCESS)
    {
        goto ERR_RET;
    }

    GYRO_LOG("mpu6515_gyro_init_client OK!\n");

#ifdef CONFIG_MPU6515_LOWPASS
    memset(&obj->fir, 0x00, sizeof(obj->fir));  
#endif

	mutex_unlock(&gyro_init_mutex);
    return MPU6515_SUCCESS;
...
EXPORT_SYMBOL(mpu6515_gyro_init_client);
/****************************************************************************** 
...
#ifdef CONFIG_COMPAT
static long compat_mpu6515_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret;
	...
	switch (cmd) {
		 ...
		 case COMPAT_GYROSCOPE_IOCTL_READ_SENSORDATA:
	       ...
	       case COMPAT_GYROSCOPE_IOCTL_SMT_DATA:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_SMT_DATA,
							(unsigned long)arg32);
			 if (ret){
			 	GYRO_ERR("GYROSCOPE_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				return ret;
		...
		case  COMPAT_GYROSCOPE_IOCTL_READ_SENSORDATA_RAW:
			if(arg32 == NULL)
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_READ_SENSORDATA_RAW,
							(unsigned long)arg32);
			 if (ret){
			 	GYRO_ERR("GYROSCOPE_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				return ret;
		...
		case COMPAT_GYROSCOPE_IOCTL_READ_TEMPERATURE:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_READ_TEMPERATURE,
							(unsigned long)arg32);
			 if (ret){
			 	GYRO_ERR("GYROSCOPE_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				return ret;
		...
		case COMPAT_GYROSCOPE_IOCTL_GET_POWER_STATUS:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_GET_POWER_STATUS,
							(unsigned long)arg32);
			 if (ret){
			 	GYRO_ERR("GYROSCOPE_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				return ret;
...
#ifndef CUSTOM_KERNEL_SENSORHUB
    err = mpu6515_gyro_init_client(client, false);
#else
...
#ifndef CUSTOM_KERNEL_SENSORHUB
    err = mpu6515_gyro_init_client(false);   
#else
    err = MPU6515_SCP_SetPowerMode(enable_status, ID_GYROSCOPE);
#endif
    if (err)
...
/*----------------------------------------------------------------------------*/
// if use  this typ of enable , Gyro only enabled but not report inputEvent to HAL
static int gyroscope_enable_nodata(int en)
{
    ...
    if((en!=0 && enable_status) || (en==0 && !enable_status)){
        printk("wanghai, gyroscope_enable_nodata return, has =%d\n", enable_status);
        return 0;
    }
    ...
    {
        printk("wanghai, gyroscope_enable_nodata will sleep a moment\n");
        return -1;
    }
...
#else
    err = MPU6515_SCP_SetPowerMode(enable_status, ID_GYROSCOPE);
    err += MPU6515_SetPWR_MGMT_2(obj_i2c_data->client, enable_status);
#endif
    mutex_unlock(&gyroscope_mutex);

    if(err != 0)
	{
		GYRO_ERR("gyroscope_enable_nodata fail!\n");
		return -1;
	}
    ...
    GYRO_LOG("gyroscope_enable_nodata OK!\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
