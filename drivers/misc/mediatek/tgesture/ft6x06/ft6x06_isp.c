...
static int ft6x06_get_vendor_from_bootloader(tinno_ts_data *ts, uint8_t *pfw_vendor, uint8_t *pfw_version)
{
	...
	//LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
 	...
 	do{
		...
		if(( FTS_CTP_FIRWARE_ID == tp_id )||(0x7918 == tp_id)){
		//rc = -EINVAL;
		//goto err;
    	    	CTP_DBG("check id OK \n");
    	   	 break;
...
void ft6x06_tp_upgrade(const char * ftbin_buf, int buf_len)
{
	...
	CTP_DBG("aijun.zhu:ic_type_flag = %d,buf_len = %d\n",ic_type_flag,buf_len);   
	//LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
	...
	CTP_DBG("entry ft6x06_tp_upgrade vl_tp_ver=%x \n",vl_tp_ver);
	if(vl_bin_ver > vl_tp_ver)
	{
		...
		CTP_DBG("yolo.....Step 2:check READ-ID...");
		...
		for( i = 0; i < 3; i++ )
		{
			...
			{
				CTP_DBG("yolo....tp_id=0x%X\n", tp_id);
	            // 5316-->7907      5206-->7903
	            //LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
				if (( 0x7908 == tp_id )||(0x7918 == tp_id)){
					CTP_DBG("check id OK \n");
					printk("yolo.....check id OK \n");
					break;
				}
			}
		}
		...
		CTP_DBG("yolo.....Step 4:erase falsh...");
		...
		if(ic_type_flag){
			...
			for(i=0; i < 200; i++)
            {
                ...
                {
                    CTP_DBG("yolo.....Step 4-1:erase falsh...");
                    break;
		   		}
                msleep(50);   
             }
		...
		CTP_DBG("yolo.....Step 6:write firmware(FW) to ctpm flash");
		...
		#if 1
		CTP_DBG("yolo.....Step 10:reset the new FW...");
	...
	{
		CTP_DBG("yolo.....tp firmware needn't upgrade. \n");
	}
