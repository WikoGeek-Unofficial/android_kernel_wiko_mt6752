...
#define PFX "OV5649_camera_sensor"
#define LOG_1 LOG_INF("OV5649,MIPI 2LANE\n")
#define LOG_2 LOG_INF("preview 1280*960@30fps,420Mbps/lane; video 1280*960@30fps,420Mbps/lane; capture 5M@15fps,420Mbps/lane\n")
/****************************   Modify end    *******************************************/
extern signed char otp_read_wb_group_ov5649(signed char index);
extern bool otp_apply_wb_ov5649(unsigned short golden_rg, unsigned short golden_bg);
...
extern int Sub_Enable(void);//器件功能验证，暂时先这么写
extern int FL_Disable(void);   

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV5649MIPI_SENSOR_ID,
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,//SENSOR_OUTPUT_FORMAT_RAW_B,//sensor output first pixel color
...
extern signed char check_sunnyMID(void);
...
static void sensor_init(void)
{
    ...
    */
    ...
    write_cmos_sensor(0x3013, 0x08);//DVDD外部供电
...
static void preview_setting(void)
{
    ...
    /********************************************************
       ...
       *
       ********************************************************/
    write_cmos_sensor(0x3820, 0x0a);//0x08 // flip off, v bin off
    write_cmos_sensor(0x3821, 0x05);//0x07 // mirror on, h bin on
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    /********************************************************
       ...
       *
       ********************************************************/
    write_cmos_sensor(0x3820, 0x42);//0x40 // flip off, v bin off
    write_cmos_sensor(0x3821, 0x04);//0x06 // mirror on, v bin off
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == OV5648MIPI_SENSOR_ID) {
                if( 0==check_sunnyMID() ) //it is not sunny module
                {	
                    LOG_INF("check_sunnyMIDFailed\n");
...
//jiangwei temp begin          
			OV5649MIPI_write_cmos_sensor(0x0103, 0x01);
			OV5649MIPI_write_cmos_sensor(0x0100, 0x01);
			mdelay(10);
		       otp_read_wb_group_ov5649(-1);
//jiangwei temp endif
...
static kal_uint32 open(void)
{
    ...
    if (OV5648MIPI_SENSOR_ID != sensor_id)
        return ERROR_SENSOR_CONNECT_FAIL;
...
#if 0
    otp_update_wb_ov5649(0x177,0x186);//sunny.待供应商确认
#else
    otp_apply_wb_ov5649(0x177,0x186);
#endif
