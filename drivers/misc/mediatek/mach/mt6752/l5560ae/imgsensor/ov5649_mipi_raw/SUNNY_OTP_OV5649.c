...
extern void OV5649MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/	
signed char otp_read_wb_group_ov5649(signed char index)
{
...
bool otp_apply_wb_ov5649(unsigned short golden_rg, unsigned short golden_bg)
{
	unsigned short gain_r = GAIN_DEFAULT_VALUE;
	unsigned short gain_g = GAIN_DEFAULT_VALUE;
	unsigned short gain_b = GAIN_DEFAULT_VALUE;
...
bool otp_update_wb_ov5649(unsigned short golden_rg, unsigned short golden_bg) 
{
	...
	if (otp_read_wb_group_ov5649(-1) != -1)
	{
		if (otp_apply_wb_ov5649(golden_rg, golden_bg) == 1)
		{
		       mdelay(1);
...
signed char check_sunnyMID(void)
	{
        signed char index;
