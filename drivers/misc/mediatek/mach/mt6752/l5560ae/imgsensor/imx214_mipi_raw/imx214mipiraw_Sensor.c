...
//	.cap_delay_frame = 3, 
	.cap_delay_frame = 0, //long time exposure.wupingzhou,20141126
	.pre_delay_frame = 3, 
	.video_delay_frame = 3,
	...
	.i2c_addr_table = {0x20,0x34, 0x6c, 0xff},
};
...
kal_uint16 sensorGainMapping[MIPI_MaxGainIndex][2] = {
...
//extern int OnReadOtpIMX214(void);
extern kal_bool IMX214_check_OTP_MID_sunny(void);
extern kal_bool IMX214_check_OTP_AF(void);
extern void IMX214_OTP_apply_awb(void);
...
void IMX214_write_cmos_sensor(u16 addr, u32 para)
{
...
static void write_shutter(kal_uint32 shutter)//wupingzhou,20140604.long time exposure
{
	kal_uint16 realtime_fps = 0;
	kal_uint32 frame_length = 0;
    kal_bool long_exposure_flag = KAL_FALSE;//long time exposure.wupingzhou,20141126
	kal_uint32 shift = 0;//long time exposure.wupingzhou,20141126
	...
	LOG_INF("shutter =%d\n", shutter);
    if(shutter>65525)//long time exposure.wupingzhou,20141126
        long_exposure_flag = KAL_TRUE;
    while (shutter>65525)//long time exposure.wupingzhou,20141126
    {
        shutter = shutter/2;
        shift = shift+1;
    }
    ...
    if(long_exposure_flag)//long time exposure.wupingzhou,20141126
    {
	    // Update Shutter
		write_cmos_sensor(0x0104, 1);      	
		write_cmos_sensor(0x0202, (shutter >> 8) & 0xFF);
		write_cmos_sensor(0x0203, shutter  & 0xFF);	
		write_cmos_sensor(0x0350, 1);  //auto frame length    	
		write_cmos_sensor(0x3028, shift);  //CIT_LSHIFT	
		write_cmos_sensor(0x0104, 0);  
    }
    else
    {
		...
		write_cmos_sensor(0x0202, (shutter >> 8) & 0xFF);
		write_cmos_sensor(0x0203, shutter  & 0xFF);	
		write_cmos_sensor(0x0350, 0);  //auto frame length    	
		write_cmos_sensor(0x3028, 0);  //CIT_LSHIFT	
		write_cmos_sensor(0x0104, 0);  
    }

//	LOG_INF("shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
	LOG_INF("shutter =%d, framelength =%d,pclk =%d, line_length =%d,shift =%d\n", shutter,imgsensor.frame_length,imgsensor.pclk,imgsensor.line_length,shift);
...
static void set_shutter(kal_uint32 shutter) //wupingzhou,20140604.long time exposure
{
	unsigned long flags;

//    write_cmos_sensor(0x0100,0x00);//STREAM OFF //long time exposure.wupingzhou,0819.打开黑屏卡死
...
//    if (imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE) {//wupingzhou,20140813.reduce the time of long time exposure
//	    mdelay(85); //fail.
//	    mdelay(110); //ok.fix capture AE problem.sometimes photo is to dark.
//    }
}	/*	set_shutter */
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    write_cmos_sensor(0x0100,0x01);//STREAM ON	 //long time exposure.wupingzhou,0819
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x4601,0x04);//0x00
    write_cmos_sensor(0x4642,0x01);//0x05
    ...
    write_cmos_sensor(0x461B,0x2C);//0x12
    ...
    write_cmos_sensor(0x465B,0x2C);//0x12
    write_cmos_sensor(0x465F,0x2B);//0x11
    write_cmos_sensor(0x4663,0x2B);//0x11
    write_cmos_sensor(0x4667,0x24);//0x0F
    write_cmos_sensor(0x466F,0x24);//0x0F   
    ...
    write_cmos_sensor(0x0101,0x00);//0x03   
    
    //OnReadOtpIMX214();
    IMX214_OTP_apply_awb();
}	/*	sensor_init  */
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				if(IMX214_check_OTP_MID_sunny())
                //IMX214_check_OTP_MID_sunny();    
				{
                    IMX214_check_OTP_AF();
...
extern void Cam_sub_ov5648_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
...
static kal_uint32 open(void)
{
	...
	if(TRUE != hwPowerOn(SUB_CAMERA_POWER_VCAM_D,VOL_1500,"DVDD"))
       {
       	LOG_INF("aijun.zhu Fail to enable digital power on\n");
            	return FALSE;
        }
	...
	Cam_sub_ov5648_write_cmos_sensor(0x3013, 0x08);//DVDD外部供电
	Cam_sub_ov5648_write_cmos_sensor(0x3018, 0x5c);//close MIPI
...
static kal_uint32 close(void)
{
	...
	if(TRUE != hwPowerDown(SUB_CAMERA_POWER_VCAM_D,"DVDD"))
       {
       	LOG_INF("aijun.zhu Fail to enable digital power down\n");
            	return FALSE;
        }
...
//			set_shutter(*feature_data_16);
			set_shutter(*feature_data_32); //wupingzhou,20140604.long time exposure
			break;
