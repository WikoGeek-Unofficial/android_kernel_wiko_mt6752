...
extern void OV5648MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/
signed char otp_read_wb_group_ov5648(signed char index)
{
	...
	if (index == 0)
	{
		// select bank 0
   		...
   		for(i = 0; i < 3; i++)
   		{
			...
			if((RG_MSB != 0)&&(BG_MSB != 0)&&(AWB_LSB != 0))
			{
				TRACE("read right  RG_MSB = %d\n",RG_MSB);
				TRACE("read right BG_MSB = %d\n",BG_MSB);
				TRACE("read right AWB_LSB = %d\n",AWB_LSB);				
				break;
			}else{
				mdelay(10);
			}
	...
	else /* (index == 1 || index ==2)*/
	{
		...
		// select bank 1
   		...
   		for(j= 0; j < 3; j++)
   		{
			...
			if((RG_MSB != 0)&&(BG_MSB != 0)&&(AWB_LSB != 0))
			{
				TRACE("read right awb RG_MSB = %d\n",RG_MSB);
				TRACE("read right awb BG_MSB = %d\n",BG_MSB);
				TRACE("read right awb AWB_LSB = %d\n",AWB_LSB);				
				break;
			}else{
				mdelay(10);
			}
...
*******************************************************************************/	
bool otp_apply_wb_ov5648(unsigned short golden_rg, unsigned short golden_bg)
{
...
// jiangwei for protect
	int rg_value = 0;
	int bg_value = 0;
	unsigned short rg_min = 0;
	unsigned short bg_min = 0;
//jiangwei end 
...
// jiangwei for protect
	if(golden_rg > rg_ratio)
		rg_min = golden_rg - rg_ratio;
	else
		rg_min = rg_ratio - golden_rg;

	if(golden_bg > bg_ratio)
		bg_min = golden_bg - bg_ratio;
	else
		bg_min = bg_ratio - golden_bg;

	if((rg_min  >  golden_rg /20)||(bg_min>  golden_bg/20)||(rg_ratio==0)||(bg_ratio==0))
	{
		TRACE("rg_ratio / bg_ratio error\n");
		return 0;		
	}
//jiangwei end 
...
bool otp_update_wb_ov5648(unsigned short golden_rg, unsigned short golden_bg) 
{
	...
	if (otp_read_wb_group_ov5648(-1) != -1)
	{
		if (otp_apply_wb_ov5648(golden_rg, golden_bg) == 1)
		{
		       mdelay(1);
...
signed char check_darlingMID(void)
	{
        signed char index;
