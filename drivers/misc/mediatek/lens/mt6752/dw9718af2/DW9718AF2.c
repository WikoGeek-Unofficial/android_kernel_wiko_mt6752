/*
 * DW9718AF2 voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "DW9718AF2"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID          0x19
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9718af2"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9718af2"
...
static int iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 * a_pRecvData, u16 a_sizeRecvData)
{
    ...
    if (i4RetValue != a_sizeSendData) {
				LOG_INF("I2C read failed!! Addr = 0x%x\n", a_pSendData[0]);
        return -1;
    }

    i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, (char *)a_pRecvData, a_sizeRecvData);
...
static kal_uint8 read_cmos_sensor_8(kal_uint32 addr)
{
    kal_uint8 get_byte=0;
    char puSendCmd[1] = {(char)(addr & 0xFF)};
    iReadRegI2C(puSendCmd , 1, (u8*)&get_byte, 1);
    return get_byte;
}
...
static int s4AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    *a_pu2Result = ((read_cmos_sensor_8(0x02) & 0x03) << 8) + (read_cmos_sensor_8(0x03) & 0xFF);
...
static int init_setting(void)
{
	...
	char sac_mode = 0x04; // 0 direct mode, 1 LSC mode, 2 SAC1 mode, 3..7  SAC2...SAC6
	char sw = 0x01;
	char drv = 0x00, swf = 0x00;
	char div = 0x01, sact = 0x20;
	char puSendCmd[2];
	...
	//step2: 0x01
	puSendCmd[0] = (char)(0x01);
	puSendCmd[1] = (char)(0x30 + ((sac_mode & 0x07) << 1 + (sw & 0x01)));
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);	
    if (i4RetValue < 0) 
    {
        LOG_INF("I2C send failed!! \n");
	...
	puSendCmd[1] = (char)((drv & 0x03) << 4 + (swf & 0x0F));
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);	
    if (i4RetValue < 0) 
    {
        LOG_INF("I2C send failed!! \n");
	...
	//step4: 0x05
	puSendCmd[0] = (char)(0x05);
	puSendCmd[1] = (char)((div & 0x03) << 6 + (sact & 0x3F));
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);	
    if (i4RetValue < 0) 
    {
        LOG_INF("I2C send failed!! \n");
...
inline static int getAFInfo(__user stDW9718AF2_MotorInfo * pstMotorInfo)
{
    stDW9718AF2_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stDW9718AF2_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stDW9718AF2_MotorMETAInfo * pstMotorMETAInfo)
{
    stDW9718AF2_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stDW9718AF2_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case DW9718AF2IOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDW9718AF2_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case DW9718AF2IOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stDW9718AF2_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case DW9718AF2IOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case DW9718AF2IOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case DW9718AF2IOC_T_SETMACROPOS :
...
static int __init DW9718AF2_i2C_init(void)
{
...
static void __exit DW9718AF2_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(DW9718AF2_i2C_init);
module_exit(DW9718AF2_i2C_exit);

MODULE_DESCRIPTION("DW9718AF2 lens module driver");
