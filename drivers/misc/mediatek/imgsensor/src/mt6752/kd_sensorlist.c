...
int iReadRegI2C_led(u8 *a_pSendData , u16 a_sizeSendData, u8 * a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    int  i4RetValue = 0;
...
int iWriteReg_led(u8 a_u2Addr , u8 a_u4Data , u8 a_u4Bytes , u16 i2cId)
{
    int  i4RetValue = 0;
    int u4Index = 0;
    u8 * puDataInBytes = (u8 *)&a_u4Data;
    ...
    char puSendCmd[6] = {(char)a_u2Addr , (char)a_u4Data , 0 , 0 , 0 , 0};
	...
	spin_unlock(&kdsensor_drv_lock);
        ...
        PK_DBG("[iWriteReg_led] jiangwei \n");
        PK_DBG("[iWriteReg_led] jiangwei  a_u4Data = %x\n",a_u4Data);
        PK_DBG("[iWriteReg_led] jiangwei  a_u4Bytes = %x\n",a_u4Bytes);
        PK_DBG("[iWriteReg_led] jiangwei  i2cId = %x\n",i2cId);
      
    if(a_u4Bytes > 1)
    {
        PK_DBG("[CAMERA SENSOR] exceed 1 bytes \n");
        return -1;
    }

    do {
		i4RetValue = i2c_master_send(g_pstI2Cclient2, puSendCmd, (a_u4Bytes + 1));
		if (i4RetValue != (a_u4Bytes + 1)) {
    ...
    for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
        ...
        if (ERROR_NONE != err)
        {
            PK_DBG("ERROR:adopt_CAMERA_HW_CheckIsAlive(), No imgsensor alive\n");
			 	...
			 	if(sensorinum++)
					     {
						     strcpy(&g_currSensorNames[0][0],&g_invokeSensorNameStr[0][0]);

						     if(sensorID==OV5648MIPI_SENSOR_ID)
						     {
								strcpy(&g_currSensorNames[0][0],"ov5648-Darling");
						     }
						     if(sensorID==OV5649MIPI_SENSOR_ID)
						     {
								strcpy(&g_currSensorNames[0][0],"ov5648-Sunny");     
						     }
					     }
					     else
					     {
					  	     strcpy(&g_currSensorNames[1][0],&g_invokeSensorNameStr[1][0]);
							 
						     if(sensorID==IMX214_SENSOR_ID)
						     {
								strcpy(&g_currSensorNames[1][0],"imx214-Sunny");
						     }
						     if(sensorID==IMX215_SENSOR_ID)
						     {
								strcpy(&g_currSensorNames[1][0],"imx214-Liteon");     
						     }
					     }
        }
        }
