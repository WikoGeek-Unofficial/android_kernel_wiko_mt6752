...
// current compensate
    ...
    if(g_fg_is_charging == KAL_TRUE)
        dvalue = ((dvalue*CHARGING_CAR_TUNE_VALUE)/100);
    else
    ...
    if (30000 < dvalue) // Current should less than 3A.
    {
        dvalue = 3700;
        bm_print(BM_LOG_CRTI, "[fgauge_read_current] error fixed current to %dmA\n", dvalue/10);
    }
...
//Auto adjust value
    ...
    {
        dvalue_CAR = ((dvalue_CAR*CHARGING_CAR_TUNE_VALUE)/100);
    } 
    #else
    dvalue_CAR = ((dvalue_CAR*CAR_TUNE_VALUE)/100);
    #endif  /* TINNO_SMART_SYNC_CAP */
