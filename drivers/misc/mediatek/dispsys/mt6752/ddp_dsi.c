...
// [JSTINNO_SRC xiaoyan.yu,  add for supporting intializing lcm in HS mode, DATE20141009-01 START
void __DSI_clk_HS_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
         int i = 0;
         for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
         {
                   if(enter)// && !DSI_clk_HS_state(i, cmdq))
                   {
                            DSI_OUTREGBIT(cmdq, DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, HSTX_CKLP_EN, 0);
                            DSI_OUTREGBIT(cmdq, DSI_PHY_LCCON_REG, DSI_REG[i]->DSI_PHY_LCCON, LC_HS_TX_EN, 1);           
                   }
                   else if (!enter)// && DSI_clk_HS_state(i, cmdq))
                   {
                            DSI_OUTREGBIT(cmdq, DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, HSTX_CKLP_EN, 1);
                            DSI_OUTREGBIT(cmdq, DSI_PHY_LCCON_REG, DSI_REG[i]->DSI_PHY_LCCON, LC_HS_TX_EN, 0);             
                   }
         }
}
 
void dsi_enter_hs(bool enter)
{
    __DSI_clk_HS_mode(DISP_MODULE_DSI0, (void*)0, enter); //clock lane enter HS mode
}
// JSTINNO_SRC xiaoyan.yu, DATE20141009-01 END]
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	for(d = DSI_MODULE_BEGIN(module);d <= DSI_MODULE_END(module);d++)
	{
		...
		do
		{
			...
			// [JSTINNO_SRC xiaoyan.yu,  add for supporting high speed read, DATE20141010-01 START
			if(DSI_clk_HS_state(DISP_MODULE_DSI0,NULL))
			t0.CONFG = 0x0C;		///BTA
			else
