...
static int NXPExtSpk_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    #endif/*TINNO_ANDROID_L8420AE*/

    //smart pa DMA support
    if (DMAbuffer_va == NULL)
    {
        DMAbuffer_va = (u8 *)dma_alloc_coherent(&client->dev, 4096, &DMAbuffer_pa, GFP_KERNEL);
    }
    if (!DMAbuffer_va)
    {
    ...
    msleep(2);
    //mt_set_gpio_mode(GPIO_SMARTPA_RST_PIN/*GPIO130*/, GPIO_MODE_00);
    //mt_set_gpio_dir(GPIO_SMARTPA_RST_PIN/*GPIO130*/, GPIO_DIR_OUT);
    ...
    //printk("client new timing=%dK \n", new_client->timing);
         ...
         //mutex_unlock(&tfa98xx_r->lock);

         printk("tfa98xx_i2c_probe:rev_value=0x%x\n", rev_value);

         if (rev_value == TFA9887_REV) 
         {
                   printk("NXP Device detected!\nTFA9887 registered I2C R driver!\n");
         }
         else if(rev_value == TFA9890_REV)
         {
                   printk("NXP Device detected!\nTFA9890 registered I2C R driver!\n");
         }
         else
         {
                   printk("NXP Device not found, i2c r error %d \n", rev_value);
                   ret = -1;
                   return ret;
         }
#endif
...
void AudDrv_NXPSpk_Init(void)
{
    ...
    mt_set_gpio_mode(GPIO_NXPSPA_I2S_LRCK_PIN , GPIO_MODE_02);
    mt_set_gpio_mode(GPIO_NXPSPA_I2S_BCK_PIN, GPIO_MODE_02);
    mt_set_gpio_mode(GPIO_NXPSPA_I2S_DATAIN_PIN, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO_NXPSPA_I2S_DATAOUT_PIN, GPIO_MODE_02);
}
...
static long AudDrv_nxpspk_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
	 // [JSTINNO_SRC xiaoyan.yu,  add case for reseting chip, DATE20141117-01 START
        case 0:
        {
            printk("AudDrv_nxpspk_ioctl reset chip\n");
            //AudDrv_NXPSpk_Init();
           ...
           mt_set_gpio_out(GPIO_SMARTPA_RST_PIN/*GPIO130*/, GPIO_OUT_ZERO);
           msleep(10);
           printk("AudDrv_nxpspk_ioctl reset chip end\n");
           #endif
            break;
        }
        // JSTINNO_SRC xiaoyan.yu, DATE20141117-01 END]
		
         /* This part do not work ok now , it is used for stereo application. */
         case I2C_SLAVE:
         case I2C_SLAVE_FORCE:
                    if(arg == TFA_I2CSLAVEBASE)
                     {
                    		 return 0;
                      }
                      else
...
static long AudDrv_nxpspk_compat_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	int  ret = 0;

	printk("AudDrv_nxpspk_compat_ioctl cmd = 0x%x arg = %lu\n", cmd, arg);
...
static ssize_t AudDrv_nxpspk_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
	...
	*/
	...
	if(count < 8)
	{
		 new_client->addr = new_client->addr & I2C_MASK_FLAG | I2C_ENEXT_FLAG;
		 ret = i2c_master_recv(new_client, Read_ptr, count);
	...
	{
		new_client->addr = new_client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG | I2C_ENEXT_FLAG;
		ret = i2c_master_recv(new_client, (unsigned char *)DMAbuffer_pa, count);
...
#ifdef CONFIG_COMPAT
    .compat_ioctl = AudDrv_nxpspk_compat_ioctl,
#endif
