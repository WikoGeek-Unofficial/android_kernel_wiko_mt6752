...
int mpu6515_init_client(int reset_cali);
int MPU6515_Dev_Reset(void);
...
static int g_mpu_test = 0;

static  atomic_t g_after_resume = ATOMIC_INIT(0);

static DEFINE_MUTEX(mpu_i2c_mutex);

static DEFINE_MUTEX(gsensor_resume_mutex);
static DEFINE_MUTEX(dev_reset_mutex);
static DEFINE_MUTEX(gs_init_mutex);


static s16 g_gs_last_data[MPU6515_AXES_NUM] = {0};
...
extern int mpu6515_gyro_init_client(bool enable); 
...
int MPU6515_i2c_master_send(u8 *buf, u8 len)
{
    ...
    {
    	mutex_lock(&mpu_i2c_mutex);
        res = i2c_master_send(mpu6515_i2c_client, buf, len);
	mutex_unlock(&mpu_i2c_mutex);
    }
...
static int MPU6515_SetPowerMode(struct i2c_client *client, bool enable)
{
    ...
    if(client == NULL){
        GSE_LOG("MPU6515_SetPowerMode client==NULL failed!\n");
        return -1;
    }
    ...
    res = MPU6515_i2c_master_send(databuf, 0x1);
    if (res <= 0)
    {
        return MPU6515_ERR_I2C;
    }
    ...
    databuf[0] = 0x0;        
    res =  MPU6515_i2c_master_recv( databuf, 1);
    if (res <= 0)
    {
        return MPU6515_ERR_I2C;
    }
#else
    databuf[0] = 0x0;
    res = mpu_i2c_read_block(client, MPU6515_REG_POWER_CTL, databuf, 0x1);
    if (res != 0)
    {
        GSE_LOG("MPU6515_SetPowerMode mpu_i2c_read_block failed!\n");
        return MPU6515_ERR_I2C;
    }
#endif
...
static int MPU6515_ReadData(struct i2c_client *client, s16 data[MPU6515_AXES_NUM])
{
    ...
    memcpy((void*)data, (void*)g_gs_last_data, (MPU6515_AXES_NUM<<1));    

	mutex_lock(&gsensor_resume_mutex);
	if(atomic_read(&(priv->suspend))==0 && atomic_read(&g_after_resume) > 0)
	{

		atomic_dec(&g_after_resume);
		mutex_unlock(&gsensor_resume_mutex);
		
		//printk("wanghai:MPU6515_ReadData()--- g_after_resume: %d\n", g_after_resume);

		MPU6515_Dev_Reset();
		mpu6515_gyro_init_client(false);
		err = mpu6515_init_client(false);

		if (err != MPU6515_SUCCESS) //0x2C->BW=100Hz
		{
			//retry next time
			printk("wanghai, MPU6515_ReadData set data format error\n");
			return err;
		}
    
	}else if (atomic_read(&g_after_resume) >0){
		mutex_unlock(&gsensor_resume_mutex);
		printk("wanghai, MPU6515_ReadData return, in suspend\n");
		return -1;
	}else{
		mutex_unlock(&gsensor_resume_mutex);
	}
    ...
    memcpy((void*)g_gs_last_data,(void*)data,(MPU6515_AXES_NUM<<1));
...
static int MPU6515_CheckDeviceID(struct i2c_client *client)
{
    ...
    if(client == NULL){
        GSE_LOG("MPU6515_CheckDeviceID client==NULL failed!\n");
        return -1;
    }
    ...
    if (res <= 0)
    {
        GSE_ERR("MPU6515_i2c_master_send failed : %d\n", res);
    ...
    databuf[0] = 0x0;        
    res =  MPU6515_i2c_master_recv( databuf, 0x01);
    if (res <= 0)
    {
        GSE_ERR("MPU6515_i2c_master_recv failed : %d\n", res);
        goto exit_MPU6515_CheckDeviceID;
    }
#else
    res = mpu_i2c_read_block(client, MPU6515_REG_DEVID, databuf, 0x1);
    if (res != 0)
    {
        GSE_ERR("MPU6515_i2c_master_recv failed : %d\n", res);
         return MPU6515_ERR_I2C;
...
static int MPU6515_REG_SetByPass(struct i2c_client *client)
{
	u8 databuf[2] = {0}; 
	u8 tempbuf;
	int smplrt_div = 0;
	...
	if(hwmsen_read_byte(client, MPU6515_ACC_REG_INT_CONFIG, databuf))
	{
	...
	databuf[0] = MPU6515_ACC_REG_INT_CONFIG;   
...
static int MPU6515_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
    ...
    if(client == NULL){
        GSE_LOG("MPU6515_SetDataFormat client==NULL failed!\n");
        return -1;
    }
...
#else
    res = mpu_i2c_read_block(client, MPU6515_REG_DATA_FORMAT, databuf, 0x1);
    if (res != 0)
    {
        return MPU6515_ERR_I2C;
    }
...
#else
    res = mpu_i2c_read_block(client, MPU6515_REG_BW_RATE, databuf, 0x1);
    if (res != 0)
    {
        return MPU6515_ERR_I2C;
    }
...
/*    if(mutex_trylock(&dev_reset_mutex)){
        printk("wanghai, another is reset dev\n");
        return 0;
    }*/    
...
#else
    res = mpu_i2c_read_block(g_client, MPU6515_REG_POWER_CTL, databuf, 0x1);
    if (res != 0)
    {
        return MPU6515_ERR_I2C;
    }
    ...
    }while(retry>0);//while((databuf[0]&MPU6515_DEV_RESET) != 0);  

    mdelay(30);
#endif //#ifndef CUSTOM_KERNEL_SENSORHUB

    // mutex_unlock(&dev_reset_mutex);
    return MPU6515_SUCCESS;    
...
int mpu6515_init_client(int reset_cali)
{
 ...
 /*   if(mutex_trylock(&gs_init_mutex)){
        printk("wanghai, another is init gyro\n");
        return 0;
    }*/
    ...
    if(g_client == NULL){
        ...
        obj = i2c_get_clientdata(g_client); 
    }
 udelay(500);

    mpu6515_gpio_config();

    res = MPU6515_SetPowerMode(g_client, true);
    ...
    res = MPU6515_CheckDeviceID(g_client); 
    if (res != MPU6515_SUCCESS)
    {
     ...
     res = MPU6515_REG_SetByPass(g_client);
     if(res != MPU6515_SUCCESS)
     {
		goto ERR_RET;
	}
    ...
    res = MPU6515_SetBWRate(g_client, MPU6515_BW_184HZ);
    ...
    res = MPU6515_SetDataFormat(g_client, MPU6515_RANGE_16G);
    if (res != MPU6515_SUCCESS) //0x2C->BW=100Hz
    {
    ...
    res = MPU6515_SetIntEnable(g_client, 0x00);//disable INT
    if (res != MPU6515_SUCCESS)
    {
    ...
    if (0 != reset_cali)
    {
        ...
        res = MPU6515_ResetCalibration(g_client);
        if (res != MPU6515_SUCCESS)
        {
            goto ERR_RET;
        }
    ...
    res = MPU6515_SetPowerMode(g_client, enable_status);
    if (res != MPU6515_SUCCESS)
    {
        GSE_ERR("set power error\n");
        goto ERR_RET;
...
static int MPU6515_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
    ...
    client = obj->client;
    int sum[3] = {0x0},i;
    #define MPU6515_DATA_FLAG		13000  
    ...
    {
        #if 1
        obj->data[MPU6515_AXIS_X] = obj->data[MPU6515_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
        obj->data[MPU6515_AXIS_Y] = obj->data[MPU6515_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
        obj->data[MPU6515_AXIS_Z] = obj->data[MPU6515_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;        
	#endif
	...
	printk("mpu6515:acc[0] = %8d,acc[1] = %8d,acc[2] = %8d\n",acc[0],acc[1],acc[2]);  
 
	//if((abs(acc[0]) >= MPU6515_DATA_FLAG) ||(abs(acc[1]) >= MPU6515_DATA_FLAG) ||(abs(acc[2]) >= MPU6515_DATA_FLAG))
	//{
		printk("mpu6515:sensor is exception\n");
		printk("mpu6515:acc[0] = %04x,acc[1] = %04x,acc[2] = %04x\n",acc[0],acc[1],acc[2]);
		for(i=0;i<10;i++)     
		{
			sum[0] += acc[MPU6515_AXIS_X];
			sum[1] += acc[MPU6515_AXIS_Y];  
			sum[2] += acc[MPU6515_AXIS_Z];

			mdelay(7);                                                 
			printk("mpu6515###:sum[0] = %04x,sum[1] = %04x,sum[2] = %04x\n",sum[0],sum[1],sum[2]);   
		}

		printk("mpu6515$$$:sum[0] = %04x,sum[1] = %04x,sum[2] = %04x\n",sum[0],sum[1],sum[2]);      
			
		acc[MPU6515_AXIS_X] = sum[0]/10;    
		acc[MPU6515_AXIS_Y] = sum[1]/10;  
		acc[MPU6515_AXIS_Z] = sum[2]/10;                                       
		printk("mpu6515xxx:acc[0] = %04x,acc[1] = %04x,acc[2] = %04x\n",acc[0],acc[1],acc[2]);  
	//}
	//else
	//{
	//	printk("mpu6515:sensor is normal\n");
	//}
	
	/*
      if((abs(acc[0]) > MPU6515_DATA_FLAG) || (abs(acc[1]) > MPU6515_DATA_FLAG) ||(abs(acc[2]) > MPU6515_DATA_FLAG))
      {
		if(abs(acc[0]) > MPU6515_DATA_FLAG)
		{
			acc[0] = acc[MPU6515_AXIS_X] - temp;
		}
		else if(abs(acc[1]) > MPU6515_DATA_FLAG)
		{
			acc[1] = acc[MPU6515_AXIS_Y] - temp;
		}
		else
		{
			acc[2] = acc[MPU6515_AXIS_Z] - temp; 
		}
	}*/
...
static ssize_t store_send(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    printk("send(%02X, %02X) = %d\n", addr, cmd,
        MPU6515_i2c_master_send(databuf, 0x2));
       ...
       unsigned long arg)
{
	...
	#else
	     cali[MPU6515_AXIS_X] = sensor_data.x;
            cali[MPU6515_AXIS_Y] = sensor_data.y;
            cali[MPU6515_AXIS_Z] = sensor_data.z;
	#endif
    ...
    case GSENSOR_IOCTL_GET_CALI:
	 ...
	 #else
		 sensor_data.x = cali[MPU6515_AXIS_X];
	        sensor_data.y = cali[MPU6515_AXIS_Y];
	        sensor_data.z = cali[MPU6515_AXIS_Z];
	 #endif
...
#ifdef CONFIG_COMPAT
static long compat_mpu6515_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long err = 0;
...
#ifndef CUSTOM_KERNEL_SENSORHUB
//wanghai add begin
//        MPU6515_Dev_Reset(NULL);
        //MPU6515_Reset(obj->client);
//wanghai add end, 20141219
//        mpu6515_gyro_init_client(false);
 //   if ((err = mpu6515_init_client(0)))    
 if ((err = MPU6515_SetPowerMode(client, enable_status)))
#else //#ifndef CUSTOM_KERNEL_SENSORHUB
...
g_mpu_test = 5;
    ...
    if (MPU6515_gyro_mode() == false)
    {
        ...
        MPU6515_Reset(obj->client);
        MPU6515_SetPowerMode(obj->client, true);
    }
...
static int gsensor_enable_nodata(int en)
{
    int err = 0;
    int loop = 2;
	struct mpu6515_i2c_data *priv = i2c_get_clientdata(mpu6515_i2c_client);  
    ...
    if((en!=0 && enable_status) || (en==0 && !enable_status)){
        printk("wanghai, gsensor_enable_nodata return, has =%d\n", enable_status);
        return 0;
    }

    enable_status = (en!=0?1:0);

    if(atomic_read(&(priv->suspend)))
    {
    	printk("wanghai, gsensor_enable_nodata will return, in suspend\n");
    	return -1;
    }
...
#else//#ifdef CUSTOM_KERNEL_SENSORHUB
    //mutex_lock(&gsensor_mutex); //wanghai del
	MPU6515_ReadSensorData(obj_i2c_data->client, buff, MPU6515_BUFSIZE);
//	mutex_unlock(&gsensor_mutex); //wanghai del on 20141219
