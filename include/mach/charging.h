...
extern int Enable_TINNO_LOG;
int __tinno_printk(const char *fmt_str, ...);

#define tinno_log_printk(num, fmt, args...) \
      do { \
        if (Enable_TINNO_LOG>=(int)num) { \
          __tinno_printk(fmt, ##args); \
        } \
      } while(0)
#else
#define tinno_log_printk(num, fmt, args...) \
...
/* Enum of charger temperature List */
typedef enum
{
    CHARGER_TEMPERATURE_LOWEST,         // < 0.C
    CHARGER_TEMPERATURE_CASE1_LOWER,    // 0.C - 15.C
    CHARGER_TEMPERATURE_NORMAL,         // 16.C - 44.C
    CHARGER_TEMPERATURE_CASE2_HIGHER,   // 45.C - 60.C
    CHARGER_TEMPERATURE_HIGHEST         // > 60.C
}HV_CHR_STATE_ENUM;
