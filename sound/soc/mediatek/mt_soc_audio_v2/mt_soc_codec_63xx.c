...
static void TurnOnDacPower(void)
{
    ...
    // [JSTINNO_SRC xiaoyan.yu,  modify for supporting real mic swap , DATE20150127-01 START
    #if 0
    ...
    if (GetAdcStatus() == false)
    {
        Ana_Set_Reg(AFE_AUDIO_TOP_CON0, 0x003a, 0x7fff);   //power on clock
    }
    else
    {
        Ana_Set_Reg(AFE_AUDIO_TOP_CON0, 0x0000, 0xffff);   //power on clock
    ...
    #endif
    // JSTINNO_SRC xiaoyan.yu, DATE20150127-01 END]
    ...
    #else
    Ana_Set_Reg(AFE_UL_DL_CON0 , 0x0001, 0x7fff); //[0] afe enable
    #endif
    // JSTINNO_SRC xiaoyan.yu, DATE20150127-01 END]
...
static bool TurnOnADcPowerDmic(int ADCType, bool enable)
{
    ...
    if (enable)
    {
        ...
        if (GetAdcStatus() == false)
        {
            ...
            if(mAudio_DMIC_swap == 1)
            {
                Ana_Set_Reg(AFE_UL_DL_CON0 , 0x8001, 0xffff); //bit15: UL L/R swap
            }
            else
            {
                Ana_Set_Reg(AFE_UL_DL_CON0 , 0x0001, 0xffff); //[0] afe enable
...
// Digital Mic L/R SWAP
static int Audio_DMIC_Swap_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    printk("%s() mAudio_DMIC_swap = %d\n", __func__, mAudio_DMIC_swap);
    ucontrol->value.integer.value[0] = mAudio_DMIC_swap;
    return 0;
}

static int Audio_DMIC_Swap_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    mAudio_DMIC_swap = ucontrol->value.integer.value[0];
    printk("%s() mAudio_DMIC_swap = %d \n", __func__, mAudio_DMIC_swap);
    return 0;
}
...
static const struct snd_kcontrol_new mt6331_UL_Codec_controls[] =
{
    ...
    SOC_ENUM_EXT("Audio_DMIC_SWAP", Audio_UL_Enum[26], Audio_DMIC_Swap_Get, Audio_DMIC_Swap_Set),
